...
from src.testing.load_testing import LoadTestRunner, LoadTestConfig, LoadTestType, LoadTestPresets
from src.testing.performance_benchmarks import PerformanceBenchmarkRunner, BenchmarkPresets, BenchmarkType
...
class PerformanceValidator:
    ...
    async def establish_baseline(self, name: str = "flask_baseline") -> PerformanceBaseline:
        ...
        logger.info(f"Baseline established: {baseline.avg_response_time_ms:.2f}ms avg response, {baseline.requests_per_second:.2f} RPS")
        
        return baseline
    
    async def validate_performance_targets(self, baseline_name: str = "flask_baseline") -> Dict[str, PerformanceValidationResult]:
    ...
    async def _run_current_performance_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            concurrency_result = await benchmark_runner.run_benchmark(concurrency_config)
            ...
            for test_result in [response_result, throughput_result, concurrency_result]:
                for result in test_result.raw_results:
                    total_requests += 1
                    if not result.success and result.error_message and 'timeout' in result.error_message.lower():
                        timeout_count += 1
            
            results['timeout_errors'] = (timeout_count / max(total_requests, 1)) * 1000  # Per 1000 requests
            
        except Exception as e:
            logger.error(f"Error during performance testing: {e}")
            # Return partial results
        
        logger.info(f"Current performance: {results['avg_response_time_ms']:.2f}ms avg, {results['requests_per_second']:.2f} RPS")
        
        return results
    
    async def _validate_single_target(self, target: PerformanceTarget, baseline: PerformanceBaseline, current: PerformanceBaseline) -> PerformanceValidationResult:
        ...
        if target == PerformanceTarget.RESPONSE_TIME_50_PERCENT:
            ...
            improvement_percentage = ((baseline.avg_response_time_ms - current.avg_response_time_ms) / baseline.avg_response_time_ms) * 100
            ...
            metrics_evidence = {
                'p95_improvement': ((baseline.p95_response_time_ms - current.p95_response_time_ms) / baseline.p95_response_time_ms) * 100,
                'p99_improvement': ((baseline.p99_response_time_ms - current.p99_response_time_ms) / baseline.p99_response_time_ms) * 100
            }
            ...
            improvement_percentage = ((current.max_concurrent_users - baseline.max_concurrent_users) / baseline.max_concurrent_users) * 100
            ...
            test_results = {
                ...
                'improvement_factor': current.max_concurrent_users / max(baseline.max_concurrent_users, 1)
            }
            
            metrics_evidence = {
                'success_rate_under_load': current.success_rate,
                'cpu_usage_efficiency': baseline.peak_cpu_percent - current.peak_cpu_percent
            }
            ...
            improvement_percentage = ((baseline.timeout_errors - current.timeout_errors) / max(baseline.timeout_errors, 1)) * 100
            ...
            improvement_percentage = ((baseline.peak_memory_mb - current.peak_memory_mb) / baseline.peak_memory_mb) * 100
            improvement_achieved = current.peak_memory_mb < baseline.peak_memory_mb * 0.8  # 20% improvement
            ...
            improvement_percentage = ((baseline.database_query_time_ms - current.database_query_time_ms) / baseline.database_query_time_ms) * 100
            improvement_achieved = current.database_query_time_ms < baseline.database_query_time_ms * 0.5  # 50% improvement
            ...
            metrics_evidence = {
                ...
                'cache_hit_improvement': current.cache_hit_rate - baseline.cache_hit_rate
            }
    ...
    async def _generate_validation_report(self, validation_results: Dict[str, PerformanceValidationResult]):
        ...
        report = {
            'validation_summary': {
                ...
                'passed_targets': sum(1 for r in validation_results.values() if r.validation_status == "PASS"),
                'partial_targets': sum(1 for r in validation_results.values() if r.validation_status == "PARTIAL"),
                'failed_targets': sum(1 for r in validation_results.values() if r.validation_status == "FAIL"),
            },
            'target_results': {},
            'overall_assessment': {},
            'recommendations': []
        ...
        logger.info(f"Summary: {overall_grade} - {passed_count}/{total_count} targets passed")
...
async def run_performance_validation(baseline_name: str = "flask_baseline") -> Dict[str, PerformanceValidationResult]:
...
async def quick_performance_check() -> Dict[str, Any]:
    ...
    return {
        ...
        'status': 'GOOD' if current_data['avg_response_time_ms'] < 500 and current_data['success_rate'] > 95 else 'NEEDS_ATTENTION'
    }
...
async def generate_performance_evidence_package() -> str:
    ...
    try:
        ...
        load_results = await run_test_suite("http://localhost:5000", f"{package_dir}/load_test_results")
        
        # Run and save benchmark results
        from src.testing.performance_benchmarks import run_benchmark_suite
        benchmark_results = await run_benchmark_suite(f"{package_dir}/benchmark_results")
