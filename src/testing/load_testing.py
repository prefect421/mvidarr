...
class LoadTestRunner:
    ...
    async def run_test(self) -> LoadTestResults:
        ...
        try:
            ...
            logger.info(
                f"Load test completed: {results.success_rate:.1f}% success rate"
            )
    ...
    async def _run_load_test(self):
        ...
        logger.info(
            f"Executing load test with {self.config.concurrent_users} concurrent users"
        )
        ...
        for user_id in range(self.config.concurrent_users):
            for request_id in range(requests_per_user):
                ...
                task = self._make_delayed_request(
                    semaphore, endpoint, request_id, user_id, delay
                )
                tasks.append(task)
    ...
    async def _run_stress_test(self):
        ...
        logger.info(
            f"Executing stress test with {self.config.concurrent_users} concurrent users"
        )
        ...
        steps = 5
        step_duration = (
            self.config.duration_seconds // steps
    ...
    async def _run_phase(self, phase_name: str, users: int, duration: int):
        ...
        while time.time() < end_time:
            ...
            task = self._make_delayed_request(
                semaphore, endpoint, request_id, user_id, 0
            )
            tasks.append(task)
            request_id += 1
    ...
    async def _make_delayed_request(
        self,
    ...
    async def _make_request(
        self, endpoint: str, request_id: int, user_id: int
    ) -> RequestResult:
    ...
    def _assess_performance(
        ...
        if peak_memory > 90:
            ...
            recommendations.append(
                "Review memory usage and implement garbage collection optimizations"
            )
        elif peak_memory > 70:
        ...
        if rps < 10:
            ...
            recommendations.append(
                "Optimize request processing and consider async improvements"
            )
...
async def run_test_suite(
    base_url: str, output_dir: str = "load_test_results"
