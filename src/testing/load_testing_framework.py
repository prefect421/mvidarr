...
class SystemMonitor:
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            ).total_seconds() if len(self.metrics) > 1 else 0
        }
    ...
    async def _make_request(self, endpoint: str) -> RequestResult:
        ...
        try:
            ...
            async with self.session.get(
                ...
                headers={"Accept": "application/json", "User-Agent": f"LoadTest-User-{self.user_id}"}
            ) as response:
                ...
                return RequestResult(
                    ...
                    timestamp=timestamp,
                    response_size_bytes=len(await response.read()) if hasattr(response, 'read') else None,
...
class LoadTestExecutor:
    ...
    async def execute_load_test(self, test_type: TestType = TestType.LOAD) -> TestMetrics:
    ...
    def _calculate_metrics(self, start_time: datetime, end_time: datetime) -> TestMetrics:
        ...
        metrics.successful_requests = sum(1 for r in self.results if 200 <= r.status_code < 400)
        ...
        metrics.p95_response_time_ms = response_times_sorted[int(len(response_times_sorted) * 0.95)]
        metrics.p99_response_time_ms = response_times_sorted[int(len(response_times_sorted) * 0.99)]
        ...
        metrics.requests_per_second = metrics.total_requests / test_duration if test_duration > 0 else 0
    ...
    async def _generate_test_report(self, metrics: TestMetrics, test_type: TestType):
        ...
        report_file = Path(self.config.results_directory) / f"{test_type.value}_report_{timestamp}.json"
        ...
        report = {
            "test_metadata": {
                ...
                "test_start": metrics.test_start_time.isoformat() if metrics.test_start_time else None,
                "test_end": metrics.test_end_time.isoformat() if metrics.test_end_time else None,
    ...
    def _assess_performance(self, metrics: TestMetrics) -> Dict[str, Any]:
        ...
        if metrics.avg_response_time_ms <= self.config.max_response_time_ms * 0.5:
            ...
            assessment["issues"].append(f"Average response time {metrics.avg_response_time_ms:.1f}ms exceeds target")
        else:
            score += 5
            assessment["criteria_scores"]["response_time"] = "POOR"
            assessment["issues"].append(f"Average response time {metrics.avg_response_time_ms:.1f}ms significantly exceeds target")
            assessment["recommendations"].append("Investigate slow endpoints and optimize database queries")
        ...
        if metrics.error_rate_percent <= self.config.max_error_rate_percent * 0.2:
            ...
            assessment["issues"].append(f"Error rate {metrics.error_rate_percent:.1f}% exceeds target")
        else:
            score += 0
            assessment["criteria_scores"]["error_rate"] = "POOR"
            assessment["issues"].append(f"Error rate {metrics.error_rate_percent:.1f}% significantly exceeds target")
            assessment["recommendations"].append("Investigate error causes and implement proper error handling")
        ...
        if metrics.requests_per_second >= self.config.min_throughput_rps * 1.2:
            ...
            assessment["issues"].append(f"Throughput {metrics.requests_per_second:.1f} RPS below target")
        else:
            score += 5
            assessment["criteria_scores"]["throughput"] = "POOR"
            assessment["issues"].append(f"Throughput {metrics.requests_per_second:.1f} RPS significantly below target")
            assessment["recommendations"].append("Consider horizontal scaling or performance optimizations")
        ...
        if cpu_ok and memory_ok:
            if metrics.max_cpu_percent <= self.config.max_cpu_percent * 0.7 and metrics.max_memory_percent <= self.config.max_memory_percent * 0.7:
                score += 25
            ...
            if not cpu_ok:
                assessment["issues"].append(f"CPU usage {metrics.max_cpu_percent:.1f}% exceeds threshold")
            if not memory_ok:
                assessment["issues"].append(f"Memory usage {metrics.max_memory_percent:.1f}% exceeds threshold")
        else:
            score += 5
            assessment["criteria_scores"]["system_resources"] = "POOR"
            assessment["issues"].append(f"Both CPU ({metrics.max_cpu_percent:.1f}%) and memory ({metrics.max_memory_percent:.1f}%) exceed thresholds")
            assessment["recommendations"].append("System resources constrained - consider scaling up or optimizing resource usage")
