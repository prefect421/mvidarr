...
class RedisConfig(BaseModel):
    ...
    socket_connect_timeout: int = Field(
        5, ge=1, le=60, description="Connection timeout"
    )
...
class SecurityConfig(BaseModel):
    ...
    session_lifetime: int = Field(
        86400, ge=300, description="Session lifetime in seconds"
    )
    password_min_length: int = Field(
        8, ge=8, le=128, description="Minimum password length"
    )
...
class StorageConfig(BaseModel):
    ...
    backup_directory: Optional[Path] = Field(
        None, description="Backup storage directory"
    )
    max_video_size: int = Field(
        5368709120, ge=1048576, description="Max video size in bytes (5GB)"
    )
...
class PerformanceConfig(BaseModel):
    ...
    worker_processes: int = Field(
        4, ge=1, le=32, description="Number of worker processes"
    )
    worker_connections: int = Field(
        1000, ge=100, le=10000, description="Connections per worker"
    )
    ...
    max_request_size: int = Field(
        104857600, ge=1048576, description="Max request size (100MB)"
    )
...
class MonitoringConfig(BaseModel):
    ...
    health_check_interval: int = Field(
        30, ge=5, le=300, description="Health check interval"
    )
...
class IntegrationConfig(BaseModel):
    ...
    spotify_client_secret: Optional[str] = Field(
        None, description="Spotify API client secret"
    )
...
class ProductionSettings(BaseSettings):
    ...
    integrations: IntegrationConfig = Field(
        default_factory=IntegrationConfig, description="Integration configuration"
    )
    ...
    def validate_database_production_settings(cls, v, values):
        ...
        if env == Environment.PRODUCTION:
            ...
            if v.max_overflow < 10:
                raise ValueError(
                    "Production database max overflow should be at least 10"
                )
    ...
    def validate_production_readiness(self) -> List[str]:
        ...
        if self.environment == Environment.PRODUCTION:
            ...
            if not self.storage.video_directory.exists():
                issues.append(
                    f"Video directory does not exist: {self.storage.video_directory}"
                )

            if not self.storage.thumbnail_directory.exists():
                issues.append(
                    f"Thumbnail directory does not exist: {self.storage.thumbnail_directory}"
                )
...
def load_production_config() -> ProductionSettings:
    ...
    try:
        ...
        if (
            not hasattr(config.security, "secret_key")
            or len(config.security.secret_key) < 32
        ):
        ...
        return ProductionSettings(
            ...
            security=SecurityConfig(secret_key=secrets.token_urlsafe(32)),
