...
class AsyncDatabaseManager:
    ...
    async def bulk_insert(self, table_name: str, records: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            logger.info(f"📦 Bulk inserted {inserted_count} records into {table_name} in {bulk_time:.3f}s")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.engine.begin() as conn:
                result = await conn.execute(text("SELECT 1 as health_check, NOW() as current_time"))
            ...
            is_healthy = (
                response_time < 1.0 and 
                pool_stats.get("utilization", 0) < 0.8 and
                self.connection_stats.get("connection_errors", 0) / max(self.connection_stats.get("queries_executed", 1), 1) < 0.01
            )
    ...
    async def close(self):
        ...
        if self.engine:
            ...
            if stats["queries_executed"] > 0:
                ...
                logger.info(f"📊 Final DB stats: {stats['queries_executed']} queries, {avg_time:.3f}s avg, {stats['slow_queries']} slow")
...
class AsyncQueryOptimizer:
    ...
    @staticmethod
    async def analyze_query_performance(query: str, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "uses_index": any("Using index" in str(row.get("Extra", "")) for row in explain_result),
                "has_filesort": any("Using filesort" in str(row.get("Extra", "")) for row in explain_result),
    ...
    async def suggest_optimizations(table_name: str) -> List[str]:
        ...
        try:
            ...
            indexes = await async_db_manager.execute_query(index_query, {"table_name": table_name})
            ...
            if table_stats:
                ...
                if stats.get("Rows", 0) > 100000:
                    suggestions.append(f"Large table {table_name} ({stats['Rows']} rows) - consider partitioning")
                
                if stats.get("Data_free", 0) > 1024 * 1024:  # 1MB
                    suggestions.append(f"Table {table_name} has fragmentation - consider OPTIMIZE TABLE")
                    
                # Check for full table scans
                if stats.get("Rows", 0) > 10000 and len(indexes) < 2:
                    suggestions.append(f"Table {table_name} may have full table scan issues - add indexes on commonly queried columns")
