...
class YouTubeImporter:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.music_keywords = [
            ...
            'official audio', 'lyrics', 'lyric video', 'acoustic',
            'live performance', 'live', 'concert', 'unplugged'
        ]
        
        # Channel patterns that typically contain music videos
        self.music_channel_patterns = [
            r'.*official.*', r'.*records.*', r'.*music.*', r'.*entertainment.*',
            r'.*vevo.*', r'.*sony.*', r'.*universal.*', r'.*warner.*'
        ]
    ...
    async def create_import_job(
        ...
        try:
            ...
            job.destination_directory = destination_directory or self.default_download_directory
            job.quality_preference = quality
            job.filter_music_only = filter_music_only
            
            # Apply options
            if options:
                job.quality_preference = VideoQuality(options.get('quality', quality.value))
                job.filter_music_only = options.get('filter_music_only', filter_music_only)
            
            # Store job
            self.active_jobs[job_id] = job
            await self._save_job_status(job)
            
            logger.info(f"Created YouTube import job {job_id} for {import_type.value}: {source_url}")
    ...
    async def start_import_job(self, job_id: str, progress_callback: Optional[Callable] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"YouTube import job {job_id} {'completed' if result['success'] else 'failed'}: "
    ...
    async def _import_playlist(self, job: ImportJob, progress_callback: Optional[Callable]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not playlist_info:
                return {'success': False, 'error': 'Failed to fetch playlist information'}
            ...
            if len(video_list) > self.max_playlist_size:
                ...
                logger.warning(f"Playlist truncated to {self.max_playlist_size} videos for consumer limits")
                ...
                try:
                    ...
                    if job.filter_music_only:
                        ...
                        if not is_likely_music:
                            logger.debug(f"Skipping non-music video: {video_info.get('title', 'Unknown')}")
                            continue
                    ...
                    if imported_video:
                        ...
                        if imported_video.local_path:
                            detection_result = await self.music_video_detector.detect_music_video(
                                imported_video.local_path
                            )
                            imported_video.is_music_video = detection_result.is_music_video
                            imported_video.music_video_confidence = detection_result.confidence.value
                    ...
                    job.progress_percent = (job.videos_processed / job.videos_found) * 100
                    
                    if progress_callback:
                        await progress_callback(job.job_id, job.progress_percent, job.current_video)
                    ...
                    logger.warning(f"Failed to import video {video_info.get('title', 'Unknown')}: {e}")
                    continue
    ...
    async def _import_single_video(self, job: ImportJob, progress_callback: Optional[Callable]) -> Dict[str, Any]:
    ...
    async def _import_channel(self, job: ImportJob, progress_callback: Optional[Callable]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, video_info in enumerate(channel_videos):
                ...
                try:
                    ...
                    logger.warning(f"Failed to import channel video {video_info.get('title', 'Unknown')}: {e}")
                    continue
    ...
    async def _import_search_results(self, job: ImportJob, progress_callback: Optional[Callable]) -> Dict[str, Any]:
        ...
        try:
            ...
            search_results = await self.youtube_service.search_videos(search_query, max_results=20)
            ...
            for i, video_info in enumerate(search_results):
                ...
                try:
                    ...
                    logger.warning(f"Failed to import search result {video_info.get('title', 'Unknown')}: {e}")
                    continue
    ...
    async def _download_video(self, video_info: Dict, destination_dir: str, quality: VideoQuality) -> Optional[ImportedVideo]:
        ...
        try:
            ...
            if video_info.get('upload_date'):
                try:
                    imported_video.upload_date = datetime.strptime(video_info['upload_date'], '%Y%m%d')
                except:
                    pass
            ...
            ytdl_opts['outtmpl'] = os.path.join(
                destination_dir,
                f"{safe_channel} - {safe_title}.%(ext)s"
            )
            ...
            with yt_dlp.YoutubeDL(ytdl_opts) as ydl:
                try:
                    ...
                    if downloaded_file and os.path.exists(downloaded_file):
                        ...
                        logger.info(f"Downloaded: {imported_video.title} ({imported_video.file_size / (1024*1024):.1f} MB)")
            ...
            logger.error(f"Failed to download video {video_info.get('title', 'Unknown')}: {e}")
            return None
    ...
    async def _get_channel_videos(self, channel_url: str, limit: int = 50) -> List[Dict]:
    ...
    async def cancel_import_job(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if job.status not in [ImportStatus.PENDING, ImportStatus.ANALYZING, ImportStatus.DOWNLOADING]:
    ...
    async def _save_job_status(self, job: ImportJob):
        ...
        try:
            ...
            await self.redis_client.setex(cache_key, 86400 * 7, json.dumps(job.to_dict()))
