...
def cleanup_old_logs(
    log_directory: str = "/app/data/logs", days_to_keep: int = 30, dry_run: bool = True
) -> Dict:
    """
...
def optimize_database() -> Dict:
    ...
    try:
        ...
        with get_db() as session:
            ...
            for (table_name,) in tables:
                try:
                    ...
                    results.append(
                        {"table": table_name, "status": "optimized", "success": True}
                    )
        ...
        return {"success": False, "error": str(e), "tables_optimized": 0}


def cleanup_orphaned_thumbnails(
    thumbnails_path: str = None, dry_run: bool = True
) -> Dict:
    """
    Remove thumbnail files for videos that no longer exist in database.

    Args:
        thumbnails_path: Path to thumbnails directory (defaults to Config.THUMBNAILS_DIR)
        dry_run: If True, only report what would be deleted
    ...
    try:
        ...
        if thumbnails_path is None:
            thumbnails_path = str(Config.THUMBNAILS_DIR)
...
def cleanup_temp_files(
    temp_paths: Optional[List[str]] = None, dry_run: bool = True
) -> Dict:
    """
    ...
    if temp_paths is None:
        temp_paths = ["/app/data/cache", "/app/data/downloads", "/tmp/mvidarr"]
...
def cleanup_old_job_history(days_to_keep: int = 30, dry_run: bool = True) -> Dict:
    """
    ...
    try:
        ...
        return {"success": False, "error": str(e), "jobs_removed": 0}
