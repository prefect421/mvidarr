...
class VevoService:
    ...
    async def _make_request(self, url: str, method: str = "GET", **kwargs) -> Optional[httpx.Response]:
        ...
        try:
            ...
            logger.error(f"❌ HTTP error {e.response.status_code} requesting Vevo URL: {url}")
            return None
        ...
        try:
            ...
            for video in videos[:limit]:
                try:
                    ...
                    logger.warning(f"Failed to enrich Vevo video {video.get('vevo_id')}: {e}")
    ...
    async def _parse_search_results(self, html_content: str, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            video_elements = soup.find_all(['div', 'article'], class_=re.compile(r'video|result|item'))
    ...
    async def _extract_video_from_element(self, element) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            thumbnail_url = img_element.get('src') or img_element.get('data-src') if img_element else None
    ...
    async def _parse_video_page(self, html_content: str, vevo_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if video_data:
                ...
                artist_element = soup.find(['span', 'div'], class_=re.compile(r'artist'))
                result['artist'] = artist_element.get_text(strip=True) if artist_element else 'Unknown Artist'
    ...
    async def get_vevo_channel_videos(
        ...
        try:
            ...
            channel_url = f"{self.base_url}/artist/{quote(artist_name.lower().replace(' ', '-'))}"
    ...
    async def _parse_channel_videos(self, html_content: str, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            video_elements = soup.find_all(['div', 'article'], class_=re.compile(r'video|item'))
    ...
    async def verify_official_video_status(self, video_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            vevo_videos = await self.search_official_music_videos(artist, title, limit=3)
    ...
    def _titles_match(self, title1: str, title2: str, threshold: float = 0.8) -> bool:
        ...
        try:
            ...
            for suffix in ['official video', 'official music video', 'music video', 'official']:
