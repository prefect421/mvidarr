                    ...
                    if video:
                        ...
                        try:
                            ...
                            if result.returncode == 0:
                                ...
                                if video_stream:
                                    ...
                                    logger.info(
                                        f"✅ Extracted technical metadata for organized video {video.id}:"
                                    )
                                    ...
                                    logger.warning(
                                        f"No video stream found in organized file: {file_path}"
                                    )
                            else:
                                logger.warning(
                                    f"ffprobe failed for organized file {file_path}: {result.stderr}"
                                )

                        except Exception as metadata_error:
                            logger.warning(
                                f"Failed to extract metadata from organized file {file_path}: {metadata_error}"
                            )
                        ...
                        try:
                            ...
                            async def enrich_metadata():
                                ...
                                if result.success:
                                    logger.info(
                                        f"Metadata enrichment completed for organized video {video.id}: "
                                        f"enriched {len(result.enriched_fields or [])} fields"
                                    )
                                else:
                                    logger.warning(
                                        f"Metadata enrichment failed for organized video {video.id}: "
                                        f"{result.errors}"
                                    )
                            ...
                            logger.info(
                                f"Triggered metadata enrichment for organized video {video.id}"
                            )

                        except Exception as enrichment_error:
                            logger.error(
                                f"Failed to trigger metadata enrichment for video {video.id}: {enrichment_error}"
                            )
                    else:
                        logger.debug(
