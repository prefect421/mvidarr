...
class VisualizationService:
    ...
    def create_line_chart(
        self, config: ChartConfig, data_points: List[Dict[str, Any]]
    ) -> ChartData:
        ...
        chart_definition = {
            ...
            "options": {
                ...
                "scales": {
                    "x": {
                        "type": "time",
                        "time": {"unit": self._get_time_unit(config.time_range)},
                        "grid": {
        ...
        if config.threshold_lines:
            chart_definition["options"]["plugins"]["annotation"] = {"annotations": {}}
            ...
            for i, threshold in enumerate(config.threshold_lines):
                chart_definition["options"]["plugins"]["annotation"]["annotations"][
                    ...
                    "borderWidth": 2,
                    "label": {
                        "content": threshold.get(
                            "label", f"Threshold: {threshold['value']}"
                        ),
                        "enabled": True,
                        "position": "end",
                    },
    ...
    def create_pie_chart(
        self, config: ChartConfig, labels: List[str], values: List[float]
    ) -> ChartData:
    ...
    def generate_performance_dashboard(
        ...
        if "system.cpu.percent" in metrics_data:
            ...
            cpu_chart = self.create_line_chart(
                cpu_config, metrics_data["system.cpu.percent"]
            )
        ...
        if "system.memory.percent" in metrics_data:
            ...
            memory_chart = self.create_gauge_chart(
                memory_config, current_memory, 100, {"warning": 80, "critical": 95}
            )
        ...
        if "app.response_time.avg_ms" in metrics_data:
            ...
            response_chart = self.create_line_chart(
                response_config, metrics_data["app.response_time.avg_ms"]
            )
