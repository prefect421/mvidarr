...
class MusicVideoDetector:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.music_keywords = {
            'official_video': ['official', 'music video', 'mv', 'videoclip', 'video clip'],
            'live_performance': ['live', 'concert', 'performance', 'tour', 'festival', 'acoustic'],
        ...
        self.filename_patterns = {
            'artist_title': re.compile(r'^([^-]+)\s*-\s*(.+?)(?:\s*\([^)]*\))?(?:\.[^.]+)?$'),
            'title_artist': re.compile(r'^(.+?)\s*-\s*([^-]+)(?:\s*\([^)]*\))?(?:\.[^.]+)?$'),
            'artist_album_title': re.compile(r'^([^-]+)\s*-\s*([^-]+)\s*-\s*(.+?)(?:\.[^.]+)?$')
        }
        
        # Audio/video indicators for music content
        self.music_indicators = {
            'audio_codec': ['aac', 'mp3', 'flac', 'ogg', 'wav'],
            'video_aspects': ['16:9', '4:3', '21:9'],  # Common music video aspect ratios
    ...
    async def detect_music_video(self, video_path: str, metadata: Optional[Dict] = None) -> MusicVideoDetectionResult:
        ...
        try:
            ...
            if result.is_music_video and result.confidence != DetectionConfidence.VERY_LOW:
                result.quality_assessment = await self._assess_music_video_quality(video_path)
            ...
            logger.info(f"Music video detection completed for {os.path.basename(video_path)}: "
    ...
    async def _gather_detection_evidence(self, video_path: str, metadata: Optional[Dict]) -> Dict[str, Any]:
    ...
    def _analyze_filename(self, filename: str) -> Dict[str, Any]:
        ...
        quality_keywords = ['hd', '720p', '1080p', '4k', 'hq', 'high quality', 'official']
    ...
    async def _get_technical_metadata(self, video_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Could not extract technical metadata for {video_path}: {e}")
            return {}
    ...
    async def _analyze_music_video_evidence(self, evidence: Dict[str, Any], video_path: str) -> MusicVideoDetectionResult:
        ...
        if self.music_indicators['duration_range'][0] <= duration <= self.music_indicators['duration_range'][1]:
            technical_score += 0.4
        ...
        result.video_type = await self._determine_video_type(evidence, result.confidence)
    ...
    async def _determine_video_type(self, evidence: Dict[str, Any], confidence: DetectionConfidence) -> MusicVideoType:
    ...
    async def _assess_music_video_quality(self, video_path: str) -> Optional[Dict]:
        ...
        try:
            ...
            quality_result = await self.quality_service.analyze_video_quality(video_path)
    ...
    async def _suggest_organization(self, result: MusicVideoDetectionResult) -> Optional[Dict]:
        ...
        if result.detected_artist:
            ...
            if result.detected_title:
                ...
                # Suggest full path
                organization['suggested_path'] = f"Music Videos/{clean_artist}/{clean_artist} - {clean_title}"
    ...
    async def batch_detect_music_videos(self, video_paths: List[str], progress_callback=None) -> List[MusicVideoDetectionResult]:
        ...
        logger.info(f"Batch music video detection completed. "
