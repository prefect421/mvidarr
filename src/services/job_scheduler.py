...
class JobScheduler:
    ...
    async def _process_ready_jobs(self):
        ...
        try:
            ...
            if scheduled_count > 0 or dependency_count > 0:
                logger.debug(
                    f"Scheduler check complete: {scheduled_count} scheduled, {dependency_count} dependency jobs processed"
                )
    ...
    async def _process_scheduled_jobs(self, job_queue) -> int:
        ...
        try:
            initial_scheduled = len(
                [j for j in job_queue._jobs.values() if j.status.value == "scheduled"]
            )
        ...
        try:
            initial_waiting = len(
                [
                    ...
                    if j.status.value == "waiting_dependencies"
                ]
            )
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "last_check": (
                self.stats["last_check"].isoformat()
                if self.stats["last_check"]
                else None
            ),
            "uptime_seconds": uptime,
            "uptime_start": (
                self.stats["uptime_start"].isoformat()
                if self.stats["uptime_start"]
                else None
            ),
        }
