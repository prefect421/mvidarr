...
class VimeoService:
    ...
    async def _make_request(
        self, url: str, method: str = "GET", use_api: bool = False, **kwargs
        ...
        try:
            ...
            logger.error(
                f"❌ HTTP error {e.response.status_code} requesting Vimeo URL: {url}"
            )
            return None
        except Exception as e:
        ...
        try:
            ...
            for video in videos[:limit]:
                try:
                    ...
                    logger.warning(
                        f"Failed to enrich Vimeo video {video.get('vimeo_id')}: {e}"
                    )
                    enriched_videos.append(video)
    ...
    async def _parse_search_results(
        ...
        try:
            ...
            video_elements = soup.find_all(
                ["div", "article"], class_=re.compile(r"video|result|clip")
            )
    ...
    async def _extract_video_from_element(self, element) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            user_element = element.find(
                ["span", "div"], class_=re.compile(r"user|author|channel")
            )
        ...
        try:
            ...
            if video_details:
                ...
                await track_media_processing_time(
                    "vimeo_video_details", processing_time
                )
        ...
        try:
            ...
            for file_info in files:
                if file_info.get("quality"):
                    ...
                    if not best_quality or self._is_better_quality(
            ...
            video_details = {
                ...
                "4k_available": any(
                    "4k" in q.lower() or "2160" in q for q in available_qualities
                ),
            }
            ...
            logger.error(
                f"❌ Failed to get Vimeo video details via API for {vimeo_id}: {e}"
            )
            return None
    ...
    async def _get_video_details_web(self, vimeo_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"❌ Failed to get Vimeo video details via web for {vimeo_id}: {e}"
            )
            return None

    async def _parse_video_page(
        self, html_content: str, vimeo_id: str
        ...
        try:
            ...
            if video_data:
                ...
                desc_element = soup.find(
                    ["div", "p"], class_=re.compile(r"description")
                )
                result["description"] = (
    ...
    def _is_better_quality(self, quality1: str, quality2: str) -> bool:
        ...
        try:
            q1_index = next(
                (i for i, q in enumerate(quality_order) if q in quality1.lower()), -1
            )
            q2_index = next(
                (i for i, q in enumerate(quality_order) if q in quality2.lower()), -1
            )
    ...
    async def find_exclusive_artist_channels(
        ...
        try:
            ...
            if self.api_token:
                ...
                response = await self._make_request(
                    search_url, params=params, use_api=True
                )
                if response:
                    api_data = response.json()
                    channels = []
                    ...
                    for user in api_data.get("data", []):
                        if artist_name.lower() in user.get("name", "").lower():
                            channel_data = {
                                ...
                                "bio": user.get("bio", ""),
                                "url": user.get("link", ""),
                                "video_count": user.get("metadata", {})
                                .get("connections", {})
                                .get("videos", {})
                                .get("total", 0),
                                "follower_count": user.get("metadata", {})
                                .get("connections", {})
                                .get("followers", {})
                                .get("total", 0),
                                "verified": user.get("account", "")
                                in ["plus", "pro", "business", "premium"],
                            }
                    ...
                    processing_time = time.time() - start_time
                    await track_media_processing_time(
                        "vimeo_find_channels", processing_time
                    )

                    logger.info(
                        f"🎥 Found {len(channels)} potential Vimeo channels for {artist_name}"
                    )
                    return channels
            ...
            logger.error(
                f"❌ Failed to find Vimeo artist channels for {artist_name}: {e}"
            )
            return []
    ...
    async def get_high_quality_video_streams(self, vimeo_id: str) -> Dict[str, Any]:
        ...
        try:
            if not self.api_token:
                logger.warning(
                    "Vimeo API token required for high quality stream access"
                )
                return {}
            ...
            if response and response.status_code == 200:
                ...
                if self.api_token:
                    api_response = await self._make_request(
                        f"{self.api_base}/me", use_api=True
                    )
                    result["api_access"] = (
