...
class UserBehaviorAnalytics:
    ...
    async def track_user_action(
        ...
        try:
            ...
            await self._update_user_session(user_id, session_id, page_url, user_agent, current_time)
    ...
    async def _cleanup_expired_sessions(self):
        ...
        for session_id in expired_sessions:
            session = self.active_sessions[session_id]
            session.total_duration_ms = int((session.last_activity - session.start_time) * 1000)
    ...
    async def _process_analytics(self):
        ...
        try:
            ...
            logger.info(f"👤 Processed analytics for {len(recent_users)} users in {processing_time:.2f}s")
    ...
    async def _calculate_user_engagement(self, user_id: str) -> UserEngagementMetrics:
        ...
        try:
            ...
            user_actions = [
                action for action in self.actions_buffer
            ...
            for session_id in sessions:
                session_actions = [a for a in user_actions if a.session_id == session_id]
                if len(session_actions) > 1:
                    ...
                    session_durations[session_id] = (end_time - start_time) / 60  # minutes
                else:
                    session_durations[session_id] = 1.0  # Default 1 minute for single-action sessions
            ...
            avg_pages_per_session = sum(pages_per_session.values()) / len(pages_per_session) if pages_per_session else 0
            ...
            # Most active hours
            hours = [int(datetime.fromtimestamp(a.timestamp).hour) for a in user_actions]
            hour_counts = defaultdict(int)
            for hour in hours:
                hour_counts[hour] += 1
            
            most_active_hours = sorted(hour_counts.keys(), key=lambda h: hour_counts[h], reverse=True)[:3]
    ...
    async def get_user_engagement_metrics(self, user_id: str) -> Optional[UserEngagementMetrics]:
        ...
        # Check cache first
        cache_manager = await get_media_cache_manager()
        cached_metrics = await cache_manager.get(CacheType.USER_DATA, f"user_engagement_{user_id}")
    ...
    async def get_user_session_info(self, session_id: str) -> Optional[UserSession]:
        ...
        cache_manager = await get_media_cache_manager()
        cached_session = await cache_manager.get(CacheType.USER_DATA, f"expired_session_{session_id}")
    ...
    async def get_user_actions(
        ...
        for action in self.actions_buffer:
            if (action.user_id == user_id and 
                ...
                (action_type is None or action.action_type == action_type)):
    ...
    async def get_popular_content(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for action in self.actions_buffer:
            if action.timestamp >= cutoff_time:
                ...
                if action.action_type in [UserActionType.VIDEO_PLAY, UserActionType.VIDEO_DOWNLOAD]:
        ...
        # Get top items
        top_videos = sorted(video_interactions.items(), key=lambda x: x[1], reverse=True)[:10]
        top_searches = sorted(search_queries.items(), key=lambda x: x[1], reverse=True)[:10]
        top_pages = sorted(popular_pages.items(), key=lambda x: x[1], reverse=True)[:10]
        
        return {
            'analysis_period_hours': hours,
            'popular_videos': [{'video_id': vid, 'interactions': count} for vid, count in top_videos],
            'popular_searches': [{'query': query, 'count': count} for query, count in top_searches],
            'popular_pages': [{'page': page, 'views': count} for page, count in top_pages],
            'total_interactions': len([a for a in self.actions_buffer if a.timestamp >= cutoff_time])
        }
    ...
    async def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_users_1h = len(set(a.user_id for a in self.actions_buffer if a.timestamp >= hour_ago))
            active_users_24h = len(set(a.user_id for a in self.actions_buffer if a.timestamp >= day_ago))
            active_users_7d = len(set(a.user_id for a in self.actions_buffer if a.timestamp >= week_ago))
            ...
            return {
                ...
                'service_health': {
                    ...
                    'buffer_utilization': len(self.actions_buffer) / self.config['max_actions_buffer']
                }
            }
    ...
    async def generate_user_report(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_actions = await self.get_user_actions(user_id, hours=168)  # Last week
            ...
            return {
                ...
                'engagement_metrics': engagement_metrics.to_dict() if engagement_metrics else None,
                'recent_activity': {
                    'total_actions_7d': len(recent_actions),
                    'action_breakdown': self._count_actions_by_type(recent_actions),
                    'most_recent_action': recent_actions[0].to_dict() if recent_actions else None
                },
                'active_sessions': user_sessions,
                'behavior_insights': self._generate_user_insights(user_id, engagement_metrics, recent_actions)
            }
    ...
    def _generate_user_insights(
        ...
        if engagement_metrics.engagement_score >= 75:
            ...
            insights.append("Low engagement - consider targeted re-engagement strategies")
        
        # Session behavior
        if engagement_metrics.avg_session_duration_minutes > 15:
            insights.append("Extended session durations indicate strong content interest")
        ...
        if engagement_metrics.searches_performed > 5:
            insights.append("Frequent searcher - ensure search functionality is optimized")
        ...
        try:
            return {
                ...
                "data_status": {
                    ...
                    "buffer_utilization_percent": (len(self.actions_buffer) / self.config['max_actions_buffer']) * 100
                },
                "capabilities": {
...
async def get_user_behavior_analytics(config: Optional[Dict[str, Any]] = None) -> UserBehaviorAnalytics:
...
async def track_video_play(user_id: str, video_id: str, session_id: str, page_url: str, user_agent: str):
...
async def track_search_query(user_id: str, query: str, results_count: int, session_id: str, page_url: str, user_agent: str):
...
async def track_page_view(user_id: str, page_url: str, user_agent: str, session_id: str, referrer: str = ""):
