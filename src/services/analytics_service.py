...
class AnalyticsService:
    ...
    async def record_metric(self, metric: MetricPoint):
        ...
        try:
            ...
            cache_key = f"metric:{metric.metric_name}:{int(metric.timestamp.timestamp())}"
    ...
    async def record_application_metrics(self, active_connections: int, 
        ...
        try:
            ...
            for metric_name, values in metrics_by_name.items():
                ...
                if len(values) >= 10:
                    ...
                    aggregations.extend([
                        (AggregationType.PERCENTILE_95, sorted_values[int(len(sorted_values) * 0.95)]),
                        (AggregationType.PERCENTILE_99, sorted_values[int(len(sorted_values) * 0.99)])
                    ])
                ...
                for agg_type, agg_value in aggregations:
                    ...
                    self.aggregated_metrics[f"{metric_name}:{agg_type.value}"].append(aggregated)
        ...
        try:
            for rule_id, rule in self.alert_rules.items():
                ...
                if rule.comparison == ">" and current_value > rule.threshold:
                    ...
                    should_alert = True
                elif rule.comparison == "==" and abs(current_value - rule.threshold) < 0.01:
                    should_alert = True
    ...
    async def _get_recent_metrics(self, metric_name: str, time_window_minutes: int) -> List[MetricPoint]:
    ...
    async def _persist_to_cache(self):
        ...
        try:
            ...
            for metric_key, aggregations in self.aggregated_metrics.items():
                if aggregations:
                    recent_aggregations = list(aggregations)[-10:]  # Last 10 aggregations
                    await self.cache_manager.set(
                        f"analytics:aggregated:{metric_key}",
                        json.dumps([asdict(agg, dict_factory=dict) for agg in recent_aggregations], default=str),
                        ttl=3600
                    )
    ...
    async def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        recent_memory_metrics = await self._get_recent_metrics("system.memory.percent", 5)
        current_memory = recent_memory_metrics[-1].value if recent_memory_metrics else 0
        
        recent_response_time = await self._get_recent_metrics("app.response_time.avg_ms", 5)
        current_response_time = recent_response_time[-1].value if recent_response_time else 0
        ...
        return {
            ...
            "system": {
                ...
                "status": "healthy" if current_cpu < 80 and current_memory < 85 else "degraded"
            },
            "application": {
    ...
    async def get_metric_history(self, metric_name: str, time_window_hours: int = 1) -> List[Dict[str, Any]]:
