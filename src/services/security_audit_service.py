...
class RiskScorer:
    ...
    def calculate_risk_score(self, event: SecurityEvent, context: Dict[str, Any] = None) -> int:
...
class SecurityAuditService:
    ...
    async def log_security_event(
        ...
        try:
            ...
            if (self.config.enable_real_time_alerts and 
                event.risk_score >= self.config.alert_threshold_score):
                await self._trigger_security_alert(event)
            
            logger.info(f"🔍 Security event logged: {event.event_type.value} (risk: {event.risk_score})")
    ...
    async def _log_to_cache(self, event: SecurityEvent):
        ...
        try:
            ...
            await self.cache_manager.set(
                ...
                ttl=self.config.cache_events_ttl
            )
    ...
    async def _log_to_file(self, event: SecurityEvent):
        ...
        try:
            log_file = self.config.log_directory / f"{self.config.log_file_prefix}_{datetime.now().strftime('%Y%m%d')}.json"
    ...
    async def _trigger_security_alert(self, event: SecurityEvent):
        ...
        try:
            ...
            await self.cache_manager.set(
                ...
                ttl=self.config.real_time_alerts_ttl
            )
            
            # Log critical alert
            logger.critical(f"🚨 SECURITY ALERT: {event.event_type.value} from {event.source_ip} (risk: {event.risk_score})")
    ...
    async def get_security_events(
        ...
        try:
            ...
            if event_types:
                ...
                conditions.append(f"event_type IN ({','.join(['?' for _ in event_type_list])})")
                params.update({f"event_type_{i}": et for i, et in enumerate(event_type_list)})
            
            if severity_levels:
                severity_list = [sl.value for sl in severity_levels]
                conditions.append(f"severity IN ({','.join(['?' for _ in severity_list])})")
                params.update({f"severity_{i}": sl for i, sl in enumerate(severity_list)})
    ...
    async def get_security_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            type_stats = await db_manager.execute_query(type_query, {"start_time": start_time})
            ...
            severity_stats = await db_manager.execute_query(severity_query, {"start_time": start_time})
            ...
            ip_stats = await db_manager.execute_query(ip_query, {"start_time": start_time})
            ...
            risk_stats = await db_manager.execute_query(risk_query, {"start_time": start_time})
            ...
            return {
                ...
                "high_risk_events": risk_stats[0]["high_risk_count"] if risk_stats else 0,
...
async def log_security_event(
    ...
    **kwargs
    ...
    return await security_audit_service.log_security_event(event_type, source_ip, **kwargs)
