...
class ArtistBrowserService:
    ...
    async def browse_artists(
        ...
        try:
            ...
            logger.info(
                f"Browsing artists: page={page}, per_page={per_page}, sort={sort_option.value}"
            )
            ...
            async with get_async_session() as session:
                ...
                base_query = (
                    select(
                        ...
                        func.count(Video.id)
                        .filter(Video.is_music_video == True)
                        .label("music_video_count"),
                ...
                if search_query and search_query.strip():
                    base_query = base_query.having(
                        Artist.name.ilike(f"%{search_query.strip()}%")
                    )
                ...
                if filters.get("music_videos_only"):
                    base_query = base_query.having(
                        func.count(Video.id).filter(Video.is_music_video == True) > 0
                    )

                if filters.get("min_video_count"):
                    base_query = base_query.having(
                        func.count(Video.id) >= filters["min_video_count"]
                    )
    ...
    async def browse_albums(
        ...
        try:
            logger.info(
                f"Browsing albums: page={page}, per_page={per_page}, sort={sort_option.value}"
            )
        ...
        try:
            ...
            async with get_async_session() as session:
                ...
                recent_videos = sorted(
                    videos, key=lambda v: v.created_at or datetime.min, reverse=True
                )[:5]
                ...
                profile = ArtistProfile(
                    ...
                    statistics={
                        "avg_video_duration": (artist_row.total_duration or 0)
                        // max(artist_row.video_count or 1, 1),
                        "avg_views_per_video": (artist_row.total_views or 0)
                        // max(artist_row.video_count or 1, 1),
                        "music_video_percentage": round(
                            (artist_row.music_video_count or 0)
                            / max(artist_row.video_count or 1, 1)
                            * 100,
                            1,
                        ),
                        "active_period_days": (
                            (
                                artist_row.last_video_date - artist_row.first_video_date
                            ).days
                            if artist_row.first_video_date
                            and artist_row.last_video_date
                            else 0
                        ),
                    },
                )
    ...
    def _sort_albums(
        self, albums: List[AlbumCard], sort_option: AlbumSortOption
    ) -> List[AlbumCard]:
        ...
        try:
            if sort_option == AlbumSortOption.TITLE_AZ:
                ...
                return sorted(
                    albums, key=lambda a: a.release_date or datetime.min, reverse=True
                )
            else:
                return sorted(albums, key=lambda a: a.title.lower())
    ...
    async def _create_artist_card(
        self, session: AsyncSession, artist_row
    ) -> ArtistCard:
        ...
        try:
            ...
            top_videos_query = (
                select(Video)
                .where(Video.artist_id == artist_row.id)
    ...
    async def _get_album_groupings(
        self, search_query: Optional[str], filters: Optional[Dict]
    ) -> List[AlbumCard]:
        ...
        try:
            ...
            async with get_async_session() as session:
                ...
                artists_query = select(Artist.id, Artist.name).where(
                    Artist.id.in_(
                ...
                if search_query:
                    artists_query = artists_query.where(
                ...
                for artist_row in artists:
                    artist_albums = await self._detect_artist_albums(
                        session, artist_row.id, artist_row.name
                    )
    ...
    async def _detect_artist_albums(
        self, session: AsyncSession, artist_id: int, artist_name: str
    ) -> List[AlbumCard]:
        ...
        try:
            ...
            for video in sorted_videos[1:]:
                if current_album_videos:
                    ...
                    if (
                        last_video_date
                        and current_video_date
                        and (current_video_date - last_video_date).days <= 30
                    ):
    ...
    async def _create_album_card(
        self, artist_id: int, artist_name: str, videos: List[Video], album_num: int
    ) -> AlbumCard:
        ...
        try:
            ...
            if len(videos) > 1:
                ...
                if first_date:
                    album_title = (
                        f"{artist_name} - {first_date.strftime('%Y')} Collection"
                    )
            else:
            ...
            return AlbumCard(
                ...
                release_date=(
                    min(v.created_at for v in videos if v.created_at)
                    if videos
                    else None
                ),
    ...
    async def _get_artist_cover_art(self, artist_name: str) -> Optional[str]:
        ...
        try:
            ...
            if os.path.exists(local_path):
                await self.redis_client.setex(
                    cache_key, self.cover_art_cache_duration, local_path
                )
                return local_path

            # Try to fetch from external sources (placeholder implementation)
            if self.enable_external_art:
                external_path = await self._fetch_external_cover_art(
                    artist_name, "artist"
                )
                if external_path:
                    await self.redis_client.setex(
                        cache_key, self.cover_art_cache_duration, external_path
                    )
            ...
            await self.redis_client.setex(
                cache_key, self.cover_art_cache_duration, placeholder_path
            )
    ...
    async def _get_album_cover_art(self, album: AlbumCard) -> Optional[str]:
        ...
        try:
            ...
            if album.tracks and len(album.tracks) > 0:
                ...
                thumbnail_path = (
                    f"/data/thumbnails/video_{first_track.get('id', 0)}_medium.jpg"
                )
    ...
    async def _fetch_external_cover_art(
        self, name: str, art_type: str
    ) -> Optional[str]:
    ...
    async def _find_similar_artists(
        self, artist_id: int, genres: List[str]
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_artist_albums(
        self, artist_id: int, videos: List[Video]
    ) -> List[AlbumCard]:
        """Get album groupings for specific artist"""
        try:
            return await self._detect_artist_albums(
                None, artist_id, videos[0].artist_name if videos else "Unknown"
            )
...
async def get_artist_browser_service(
    config: Optional[Dict] = None,
) -> ArtistBrowserService:
