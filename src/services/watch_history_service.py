...
class WatchHistoryService:
    ...
    async def update_watch_progress(
        self, session_id: str, current_time: int, total_time: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                5 <= progress_percent < 95
                and current_time >= self.minimum_watch_duration
            ):
                await self._add_to_continue_watching(session)

            logger.debug(
                f"Updated watch progress for session {session_id}: {progress_percent:.1f}%"
            )
    ...
    async def end_watch_session(self, session_id: str) -> WatchSession:
        ...
        try:
            ...
            if session.started_at:
                ...
                session.watch_duration = min(
                    session.watch_duration, int(total_session_time)
                )

            # Final progress calculation
            if session.video_duration > 0:
                session.progress_percent = min(
                    100.0, (session.watch_duration / session.video_duration) * 100
                )
            ...
            if session.is_completed:
                await self._remove_from_continue_watching(
                    session.video_id, session.user_id
                )
            else:
            ...
            logger.info(
                f"Ended watch session {session_id}, {session.progress_percent:.1f}% watched"
            )
            ...
            if cached_data:
                ...
                async with get_async_session() as session:
                    for item_data in continue_watching_data[:limit]:
                        video_query = (
                            select(Video)
                            .where(Video.id == item_data["video_id"])
        ...
        try:
            ...
            if cached_data:
                ...
                history = VideoWatchHistory(
                    ...
                    sessions=sessions,
                    favorite_marked_at=(
                        datetime.fromisoformat(history_data["favorite_marked_at"])
                        if history_data.get("favorite_marked_at")
                        else None
                    ),
                )
    ...
    async def get_watch_statistics(
        ...
        try:
            ...
            completed_sessions = sum(
                1 for s in sessions if s.get("is_completed", False)
            )
            completion_rate = (
            ...
            avg_session_length = (
                int(sum(session_lengths) / len(session_lengths))
                if session_lengths
                else 0
            )
            ...
            most_active_hours = sorted(
                hours_activity.keys(), key=lambda x: hours_activity[x], reverse=True
            )[:3]
            ...
            last_active = (
                max(datetime.fromisoformat(s["started_at"]) for s in sessions)
                if sessions
                else datetime.now()
            )
    ...
    async def clear_continue_watching(
        self, user_id: int, video_ids: Optional[List[int]] = None
    ) -> bool:
        ...
        try:
            ...
            if video_ids:
                ...
                if cached_data:
                    ...
                    filtered_data = [
                        item
                        for item in continue_watching_data
                        if item["video_id"] not in video_ids
                    ]
                    await self.redis_client.setex(
                        cache_key, 86400 * 7, json.dumps(filtered_data)
                    )
            else:
                # Clear all
    ...
    async def _save_watch_session(self, session: WatchSession):
        ...
        try:
            ...
            # Store for 7 days
            await self.redis_client.setex(
                session_key, 86400 * 7, json.dumps(session_data)
            )
    ...
    async def _update_video_history(self, session: WatchSession):
        """Update video watch history"""
        try:
            cache_key = f"video_history:{session.video_id}" + (
                f":{session.user_id}" if session.user_id else ""
            )

            # Load existing history
            existing_history = await self.get_video_watch_history(
                session.video_id, session.user_id
            )
            ...
            if existing_history:
                ...
                existing_history.sessions = sorted(
                    existing_history.sessions, key=lambda x: x.started_at, reverse=True
                )[:10]
            ...
            history_data = {
                ...
                "favorite_marked_at": (
                    existing_history.favorite_marked_at.isoformat()
                    if existing_history.favorite_marked_at
                    else None
                ),
            }

            await self.redis_client.setex(
                cache_key, 86400 * self.history_retention_days, json.dumps(history_data)
            )
    ...
    async def _add_to_continue_watching(self, session: WatchSession):
        ...
        try:
            ...
            continue_watching_data = [
                item
                for item in continue_watching_data
                if item["video_id"] != session.video_id
            ]
            ...
            # Save updated list
            await self.redis_client.setex(
                cache_key, 86400 * 7, json.dumps(continue_watching_data)
            )

        except Exception as e:
            logger.error(f"Failed to add to continue watching: {e}")

    async def _remove_from_continue_watching(
        self, video_id: int, user_id: Optional[int]
    ):
