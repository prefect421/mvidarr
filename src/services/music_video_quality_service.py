...
class MusicVideoQualityService:
    ...
    async def analyze_music_video_quality(
        ...
        try:
            ...
            metrics.audio_quality_score = await self._assess_audio_quality(metadata, video_path)
            metrics.technical_quality_score = await self._assess_technical_quality(metadata)
            
            # Detect video type and content quality
            metrics.video_type = await self._detect_video_type(video_path, artist, title)
            metrics.content_quality_score = await self._assess_content_quality(video_path, metrics.video_type)
            ...
            metrics.quality_rating = self._determine_quality_rating(metrics.overall_quality_score)
            ...
            self.stats['average_score'] = (
                (self.stats['average_score'] * (self.stats['assessments_completed'] - 1) + 
                 metrics.overall_quality_score) / self.stats['assessments_completed']
            )
            self.stats['quality_distribution'][metrics.quality_rating.value] += 1
            
            # Track performance
            await track_media_processing_time("music_video_quality_analysis", processing_time)
    ...
    async def _extract_detailed_metadata(self, video_path: str) -> Optional[Dict[str, Any]]:
    ...
    async def _assess_audio_quality(self, metadata: Dict[str, Any], video_path: str) -> float:
    ...
    async def _detect_video_type(
        ...
        try:
            ...
            if any(keyword in filename for keyword in ['live', 'concert', 'performance']):
            ...
            elif any(keyword in filename for keyword in ['interview', 'behind', 'making']):
    ...
    async def _assess_content_quality(self, video_path: str, video_type: VideoType) -> float:
    ...
    async def _check_music_video_issues(self, video_path: str, metrics: VideoQualityMetrics):
        """Check for specific music video issues"""
        try:
    ...
    def _calculate_overall_score(self, metrics: VideoQualityMetrics) -> float:
        ...
        try:
            total_score = (
                ...
                metrics.technical_quality_score * self.quality_weights['technical_quality'] +
                metrics.content_quality_score * self.quality_weights['content_quality']
            )
    ...
    async def _generate_recommendations(self, metrics: VideoQualityMetrics) -> List[str]:
        ...
        try:
            ...
            if metrics.video_quality_score < 60:
                if 'x' in metrics.resolution:
                    ...
                    if width < 1280:
                        recommendations.append("Consider finding a higher resolution version (at least 720p)")
            
            # Audio recommendations
            if metrics.audio_quality_score < 60:
                recommendations.append("Audio quality could be improved - look for higher bitrate version")
            
            # Codec recommendations
            if metrics.video_codec in ['mpeg2', 'mpeg1', 'wmv']:
                recommendations.append("Video uses outdated codec - modern H.264/H.265 would be better")
            
            # Duration recommendations
            if metrics.duration < 60:
                recommendations.append("Video seems too short - verify this is the complete music video")
            elif metrics.duration > 600:
                recommendations.append("Video is quite long - consider if this is the official video or extended version")
            
            # File size recommendations
            file_size_mb = metrics.file_size / (1024 * 1024)
            if file_size_mb > 500:  # Very large file
                recommendations.append("File size is quite large - consider compression if storage is limited")
            elif file_size_mb < 10 and metrics.duration > 120:  # Very small file for duration
                recommendations.append("File size seems small for duration - may indicate low quality encoding")
    ...
    async def get_quality_statistics(self) -> Dict[str, Any]:
        ...
        try:
            avg_processing_time = (
                self.stats['total_processing_time'] / max(1, self.stats['assessments_completed'])
            )
