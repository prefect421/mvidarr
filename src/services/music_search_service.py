...
class FacetFilter:
    ...
    name: str
    label: str
    values: List[Dict[str, Any]]  # [{'value': 'pop', 'count': 15, 'label': 'Pop Music'}]
...
class MusicSearchService:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.music_patterns = {
            'artist_featuring': re.compile(r'(.+?)\s+(?:ft\.?|feat\.?|featuring)\s+(.+)', re.IGNORECASE),
            ...
            'official_video': re.compile(r'(.+?)\s+official\s+(?:music\s+)?video', re.IGNORECASE)
        }
    ...
    async def search(
        ...
        try:
            ...
            logger.info(f"Music search: query='{query}', type={search_type.value}, scope={search_scope.value}")
            
            # Check cache first
            cache_key = self._generate_cache_key(query, search_type, search_scope, filters, sort_by, page, per_page)
            ...
            async with get_async_session() as session:
                ...
                search_conditions = await self._build_search_conditions(enhanced_query, search_type, search_scope)
                ...
                # Apply sorting
                base_query = await self._apply_search_sorting(base_query, sort_by, enhanced_query)
                ...
                search_results = []
                for video in videos:
                    search_result = await self._create_music_search_result(video, enhanced_query, search_type)
                ...
                facets = []
                if include_facets:
                    facets = await self._build_music_facets(session, enhanced_query, search_conditions, filter_conditions)
                
                # Get music-specific suggestions and corrections
                suggestions = []
                corrections = None
                if include_suggestions:
                    suggestions = await self._get_music_suggestions(query, search_results)
                ...
                logger.info(f"Music search completed: {len(search_results)} results in {search_time:.1f}ms")
    ...
    async def _enhance_music_query(self, query: str, search_type: SearchType) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern_name, pattern in self.music_patterns.items():
                ...
                if match:
                    ...
                    elif pattern_name in ['title_remix', 'title_version', 'live_performance']:
            ...
            decade_patterns = [r'(\d{4})s?', r'(nineteen|twenty)\s*(sixties|seventies|eighties|nineties)']
            ...
            if not enhanced['artist'] and not enhanced['title']:
                enhanced['terms'] = [term.strip() for term in query.split() if len(term.strip()) > 2]
    ...
    async def _build_search_conditions(self, enhanced_query: Dict, search_type: SearchType, search_scope: SearchScope) -> List:
        ...
        try:
            ...
            if search_type == SearchType.ARTIST and enhanced_query.get('artist'):
                conditions.append(Video.artist_name.ilike(f"%{enhanced_query['artist']}%"))
                ...
                if enhanced_query.get('artist'):
                    general_conditions.append(Video.artist_name.ilike(f"%{enhanced_query['artist']}%"))
                
                if enhanced_query.get('title'):
                    general_conditions.append(Video.title.ilike(f"%{enhanced_query['title']}%"))
                ...
                if field == 'duration' and operator in ['gte', 'lte']:
                    ...
                    if quality_range:
                        if 'max_height' in quality_range:
                            ...
                            conditions.append(Video.height >= quality_range['min_height'])
    ...
    async def _create_music_search_result(self, video: Video, enhanced_query: Dict, search_type: SearchType) -> SearchResult:
    ...
    async def _calculate_music_match_score(self, video: Video, enhanced_query: Dict) -> float:
        ...
        try:
            ...
            if enhanced_query.get('artist') and enhanced_query['artist'].lower() in artist:
                score += 0.5
            ...
            if video.is_music_video and video.created_at and (datetime.now() - video.created_at).days < 30:
                score += 0.05
    ...
    async def _generate_music_highlights(self, video: Video, enhanced_query: Dict) -> Dict[str, List[str]]:
    ...
    async def _build_music_facets(self, session: AsyncSession, enhanced_query: Dict, search_conditions: List, filter_conditions: List) -> List[FacetFilter]:
        ...
        try:
            ...
            artist_facet = await self._build_music_artist_facet(session, base_conditions)
            if artist_facet:
                facets.append(artist_facet)
            
            # Music video type facet
            music_video_facet = await self._build_music_video_facet(session, base_conditions)
            ...
            duration_facet = await self._build_music_duration_facet(session, base_conditions)
            if duration_facet:
    ...
    async def _build_music_artist_facet(self, session: AsyncSession, conditions: List) -> Optional[FacetFilter]:
        """Build music artist facet with music video counts"""
        try:
            query = select(Video.artist_name, func.count(Video.id).label('count')).group_by(Video.artist_name)
            
            if conditions:
                query = query.where(and_(*conditions))
            
            # Focus on artists with music videos
            query = query.having(Video.artist_name.isnot(None)).order_by(desc(func.count(Video.id))).limit(15)
    ...
    async def _build_music_video_facet(self, session: AsyncSession, conditions: List) -> Optional[FacetFilter]:
        """Build music video type facet"""
        try:
            values = []
            
            # Count music videos
            music_query = select(func.count(Video.id)).where(Video.is_music_video == True)
            if conditions:
                music_query = music_query.where(and_(*conditions))
            music_count = await session.scalar(music_query)
            
            # Count non-music videos
            non_music_query = select(func.count(Video.id)).where(Video.is_music_video == False)
            if conditions:
    ...
    async def _build_genre_facet(self, session: AsyncSession, conditions: List) -> Optional[FacetFilter]:
    ...
    async def _build_music_duration_facet(self, session: AsyncSession, conditions: List) -> Optional[FacetFilter]:
    ...
    async def _build_quality_facet(self, session: AsyncSession, conditions: List) -> Optional[FacetFilter]:
        """Build quality facet"""
        try:
            # Get height distribution
            query = select(Video.height, func.count(Video.id).label('count')).group_by(Video.height)
    ...
    async def _get_music_suggestions(self, query: str, results: List[SearchResult]) -> List[str]:
        ...
        try:
            ...
            if not any(keyword in query.lower() for keyword in ['official', 'video', 'live', 'acoustic', 'remix']):
                suggestions.extend([
            ...
            for genre, keywords in self.genre_keywords.items():
                if any(keyword in query_lower for keyword in keywords[:2]):  # Check first 2 keywords
    ...
    def _generate_cache_key(self, query: str, search_type: SearchType, search_scope: SearchScope, 
                          filters: List[SearchFilter], sort_by: SortBy, page: int, per_page: int) -> str:
