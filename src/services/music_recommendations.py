...
class MusicRecommendationService:
    ...
    async def get_recommendations(self, request: RecommendationRequest) -> RecommendationResult:
        ...
        try:
            ...
            for rec_type in request.recommendation_types:
                ...
                if rec_type == RecommendationType.SIMILAR_ARTISTS and request.artist_name:
                    recs = await self._get_similar_artist_recommendations(
                        request.artist_name, request.max_recommendations // len(request.recommendation_types)
                    )
                    ...
                    recs = await self._get_user_based_recommendations(
                        request.user_id, request.max_recommendations // len(request.recommendation_types)
                    )
                    all_recommendations.extend(recs)
                    ...
                    recs = await self._get_genre_based_recommendations(
                        request.genre, request.max_recommendations // len(request.recommendation_types)
                    )
                    all_recommendations.extend(recs)
            ...
            unique_recommendations = await self._deduplicate_and_rank(all_recommendations)
    ...
    async def _get_similar_artist_recommendations(self, artist_name: str, max_count: int) -> List[RecommendationItem]:
        ...
        try:
            ...
            if self.config["enable_lastfm"]:
                ...
                similar_artists = await asyncio.to_thread(lastfm_service.get_similar_artists, artist_name)
                ...
                if self.config["enable_imvdb"]:
                    ...
                    for similar_artist in similar_artists[:5]:  # Top 5 similar artists
                        try:
                            ...
                            videos_response = await asyncio.to_thread(imvdb_service.search_artist_videos, similar_artist)
                            videos = videos_response.get('videos', []) if isinstance(videos_response, dict) else []
                            
                            for video in videos[:2]:  # Top 2 videos per artist
                                recommendations.append(RecommendationItem(
                                    video_id=str(video.get('id', '')),
                                    title=video.get('song_title', 'Unknown'),
                                    artist_name=video.get('artist_name', similar_artist),
                                    ...
                                    reasons=[f"Similar to {artist_name}", f"Popular video by {similar_artist}"],
                                    metadata={
                            ...
                            logger.warning(f"Failed to get videos for {similar_artist}: {e}")
            ...
            if self.config["enable_spotify"] and len(recommendations) < max_count:
                try:
                    ...
                    for track in spotify_recs.get('tracks', [])[:max_count - len(recommendations)]:
                        ...
                        if self.config["enable_imvdb"]:
                            imvdb_service = await get_imvdb_service()
                            videos = await asyncio.to_thread(imvdb_service.search_videos, track_artist, track_name)
                            ...
                            if videos:
                                ...
                                recommendations.append(RecommendationItem(
                                    ...
                                    reasons=[f"Spotify recommends based on {artist_name}"],
                                    metadata={
    ...
    async def _get_trending_video_recommendations(self, max_count: int) -> List[RecommendationItem]:
        ...
        try:
            if self.config["enable_imvdb"]:
                ...
                # Get trending videos
                trending_videos = await asyncio.to_thread(imvdb_service.get_trending_videos, 7, max_count)
                ...
                for video in trending_videos[:max_count]:
                    recommendations.append(RecommendationItem(
                        ...
                        relevance_score=video.get('views', 0) / 10000.0,  # Views-based relevance
    ...
    async def _get_user_based_recommendations(self, user_id: str, max_count: int) -> List[RecommendationItem]:
        ...
        try:
            if self.config["enable_spotify"]:
                ...
                top_artists = await asyncio.to_thread(spotify_service.get_user_top_artists, user_id, limit=10)
                ...
                for artist in top_artists.get('items', []):
                    ...
                    if self.config["enable_imvdb"]:
                        imvdb_service = await get_imvdb_service()
                        videos_response = await asyncio.to_thread(imvdb_service.search_artist_videos, artist_name)
                        videos = videos_response.get('videos', []) if isinstance(videos_response, dict) else []
                        ...
                        for video in videos[:2]:  # Top 2 videos per artist
                            recommendations.append(RecommendationItem(
                                ...
                                relevance_score=artist.get('popularity', 50) / 100.0,
                                recommendation_type=RecommendationType.USER_BASED,
                                source=RecommendationSource.SPOTIFY,
                                reasons=[f"Based on your listening history", f"You listen to {artist_name}"],
                                metadata={
    ...
    async def _get_genre_based_recommendations(self, genre: str, max_count: int) -> List[RecommendationItem]:
        ...
        try:
            if self.config["enable_imvdb"]:
                ...
                videos = await asyncio.to_thread(imvdb_service.search_videos_by_genre, genre, max_count)
    ...
    async def _get_new_release_recommendations(self, max_count: int) -> List[RecommendationItem]:
        ...
        try:
            if self.config["enable_spotify"]:
                ...
                new_releases = await asyncio.to_thread(spotify_service.get_new_releases, limit=20)
    ...
    async def _deduplicate_and_rank(self, recommendations: List[RecommendationItem]) -> List[RecommendationItem]:
    ...
    def _update_recommendation_stats(self, result: RecommendationResult, sources_used: List[str]):
...
async def get_music_recommendation_service(config: Optional[Dict[str, Any]] = None) -> MusicRecommendationService:
...
async def get_music_recommendations(
    ...
    request = RecommendationRequest(
        ...
        user_id=user_id,
        genre=genre,
        recommendation_types=recommendation_types or [RecommendationType.SIMILAR_ARTISTS],
        max_recommendations=max_recommendations
    )
    return await service.get_recommendations(request)


async def get_similar_artist_videos(artist_name: str, max_count: int = 10) -> RecommendationResult:
