...
class VideoBrowserService:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.view_configurations = {
            ViewMode.GRID_LARGE: {'per_row': 3, 'thumbnail_size': 'large', 'show_details': True},
            ViewMode.GRID_MEDIUM: {'per_row': 4, 'thumbnail_size': 'medium', 'show_details': True},
            ViewMode.GRID_SMALL: {'per_row': 6, 'thumbnail_size': 'small', 'show_details': False},
            ViewMode.LIST_DETAILED: {'per_row': 1, 'thumbnail_size': 'medium', 'show_details': True},
            ViewMode.LIST_COMPACT: {'per_row': 1, 'thumbnail_size': 'small', 'show_details': False}
        }
    ...
    async def browse_videos(
        ...
        try:
            ...
            logger.info(f"Browsing videos: page={page}, per_page={per_page}, sort={sort_option.value}, view={view_mode.value}")
            ...
            async with get_async_session() as session:
                ...
                count_query = await self._apply_filters(count_query, filters, search_query)
                ...
                logger.info(f"Browser query returned {len(thumbnails)} videos out of {total_count} total")
    ...
    async def _apply_filters(self, query, filters: Dict[str, Any], search_query: Optional[str]):
    ...
    def _format_quality_display(self, width: Optional[int], height: Optional[int]) -> str:
        ...
        try:
            async with get_async_session() as session:
                ...
                return {
                    ...
                    'music_video_percentage': round((music_videos or 0) / max(total_videos or 1, 1) * 100, 1),
                    'recent_videos_week': recent_videos or 0,
                    'quality_distribution': quality_stats,
                    'file_size_stats': {
                        'average_mb': round((size_row.avg_size or 0) / (1024 * 1024), 1),
                        'total_gb': round((size_row.total_size or 0) / (1024 * 1024 * 1024), 2),
                        'smallest_mb': round((size_row.min_size or 0) / (1024 * 1024), 1),
                        'largest_mb': round((size_row.max_size or 0) / (1024 * 1024), 1)
                    },
        ...
        try:
            async with get_async_session() as session:
                ...
                large_count = await session.scalar(
                    select(func.count(Video.id)).where(Video.file_size > 100 * 1024 * 1024)
                )
                filters['large_files'] = {
    ...
    async def save_user_preferences(self, user_id: str, preferences: BrowserPreferences):
        ...
        try:
            ...
            if cached_prefs:
                ...
                return BrowserPreferences(
                    default_view_mode=ViewMode(prefs_dict.get('default_view_mode', ViewMode.GRID_MEDIUM.value)),
                    default_sort=SortOption(prefs_dict.get('default_sort', SortOption.RECENTLY_ADDED.value)),
                    default_per_page=prefs_dict.get('default_per_page', 24),
                    show_music_videos_only=prefs_dict.get('show_music_videos_only', False),
    ...
    async def _cache_browser_result(self, result: BrowserResult, filters: Dict, search_query: Optional[str]):
        ...
        try:
            ...
            await self.redis_client.setex(cache_key, self.cache_duration, json.dumps(cache_data))
...
async def get_video_browser_service(config: Optional[Dict] = None) -> VideoBrowserService:
