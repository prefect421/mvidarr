...
class VideoFingerprintingService:
    ...
    async def generate_video_fingerprint(
        ...
        try:
            ...
            cached_fingerprint = await cache_manager.get(CacheType.MEDIA_METADATA, cache_key)
            ...
            if self.config['enable_audio_fingerprinting']:
                fingerprint.audio_fingerprint = await self._generate_audio_fingerprint(video_path)
            ...
            logger.info(f"✅ Generated fingerprint for {video_path_obj.name} in {processing_time:.2f}s")
            return fingerprint
            
        except Exception as e:
            logger.error(f"❌ Failed to generate video fingerprint for {video_path}: {e}")
            return None
    ...
    async def _extract_video_metadata(self, video_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                cmd = [
                    ...
                    '-ss', '30',  # 30 seconds in
                    '-vframes', '1',
                    '-vf', 'scale=64:64',  # Small scale for hashing
                    temp_frame_path
                ]
    ...
    async def detect_duplicate_versions(
        ...
        try:
            logger.info(f"🔍 Checking for duplicates of video: {video_fingerprint.video_id}")
            ...
            for candidate_id in similar_duration_videos:
                ...
                match = await self._compare_fingerprints(video_fingerprint, candidate_fp)
            ...
            logger.info(f"🔍 Found {len(matches)} potential duplicates in {processing_time:.2f}s")
            return matches
    ...
    async def _compare_fingerprints(
        ...
        try:
            ...
            duration_similarity = max(0, 1 - (duration_diff / max(fp1.duration, fp2.duration)))
            ...
            if fp1.resolution == fp2.resolution:
                ...
                differences.append(f"Resolution difference: {fp1.resolution} vs {fp2.resolution}")
    ...
    async def _group_video_versions(self, videos: List[VideoFingerprint]) -> List[Dict[str, Any]]:
