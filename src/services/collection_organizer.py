...
class CollectionOrganizer:
    ...
    async def create_organization_plan(
        ...
        try:
            ...
            logger.info(
                f"Creating organization plan for {source_directory} -> {target_directory}"
            )
            ...
            logger.info(
                f"Analyzing {len(video_files)} video files for music video content"
            )
            detection_results = await self._batch_detect_videos(video_files)

            # Process each detected music video
            music_videos = []
            for i, (video_path, detection_result) in enumerate(
                zip(video_files, detection_results)
            ):
            ...
            logger.info(
                f"Found {plan.music_videos_found} music videos out of {plan.total_files} files"
            )
            ...
            for video_path, detection_result in music_videos:
                ...
                if action:
                    ...
                    conflict = await self._check_organization_conflict(
                        action, plan.organization_actions[:-1]
                    )
                    if conflict:
            ...
            low_confidence_count = sum(
                1
                for _, result in zip(video_files, detection_results)
                if result.confidence.value in ["low", "very_low"]
                and result.is_music_video
            )

            if low_confidence_count > 0:
                plan.warnings.append(
                    f"{low_confidence_count} files have low confidence music video detection"
                )

            logger.info(
                f"Organization plan created: {plan.music_videos_found} music videos, "
    ...
    async def execute_organization_plan(
        self, plan: OrganizationPlan, dry_run: bool = False, progress_callback=None
    ) -> OrganizationResult:
        ...
        try:
            logger.info(
                f"Executing organization plan: {len(plan.organization_actions)} actions "
                f"(dry_run={dry_run})"
            )
            ...
            for i, action in enumerate(plan.organization_actions):
                try:
                    ...
                    action_result = await self._execute_organization_action(
                        action, dry_run, created_folders
                    )
                    ...
                    if progress_callback:
                        await progress_callback(
                            i + 1, len(plan.organization_actions), action
                        )
                    ...
                    logger.error(
                        f"Failed to execute action {action.get('source_path', 'unknown')}: {e}"
                    )
            ...
            logger.info(
                f"Organization plan execution completed: "
    ...
    async def organize_collection(
        ...
        try:
            # Create plan
            plan = await self.create_organization_plan(
            ...
            # Execute plan
            return await self.execute_organization_plan(
                plan, dry_run, progress_callback
            )
    ...
    async def _batch_detect_videos(
    ...
    async def _generate_target_path(
        ...
        try:
            ...
            if rules.strategy == OrganizationStrategy.ARTIST_TITLE:
                ...
                filename_parts = (
                    [artist, title] if not rules.create_artist_folders else [title]
                )
                ...
                genre = (
                    detection_result.detected_genres[0]
                    if detection_result.detected_genres
                    else "Unknown Genre"
                )
                if rules.sanitize_names:
                ...
                filename_parts = (
                    [genre, artist, title]
                    if not rules.create_artist_folders
                    else [title]
                )
                ...
                path_components = (
                    [str(year), artist] if rules.create_artist_folders else []
                )
                filename_parts = (
                    [str(year), artist, title]
                    if not rules.create_artist_folders
                    else [title]
                )
                ...
                if rules.custom_pattern:
                    ...
                    custom_path = rules.custom_pattern.format(
                        artist=artist, title=title, year=datetime.now().year
                    )
    ...
    async def _check_organization_conflict(
        self, action: Dict[str, Any], existing_actions: List[Dict[str, Any]]
    ) -> Optional[Dict[str, Any]]:
    ...
    async def _execute_organization_action(
        self, action: Dict[str, Any], dry_run: bool, created_folders: Set[str]
    ) -> Dict[str, Any]:
        """Execute a single organization action"""
        result = {"moved": False, "renamed": False, "folder_created": False}
...
async def get_collection_organizer(
