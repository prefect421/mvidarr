...
logger = get_logger("mvidarr.services.emby")


class EmbyService(BaseMediaServerService):
    """Service for Emby Media Server integration"""
    ...
    def __init__(self):
        ...
        self.user_id = None
        self.device_id = "mvidarr-emby-client"
        self.device_name = "MVidarr"
        self.client_name = "MVidarr"
        self.client_version = "1.0.0"

        # Set up session headers (Emby uses X-Emby headers like Jellyfin)
    ...
    def _load_config(self) -> MediaServerConfig:
        """Load Emby configuration from settings"""
        try:
            server_url = SettingsService.get("emby_server_url", "http://localhost:8096")
            username = SettingsService.get("emby_username")
            password = SettingsService.get("emby_password")
            api_key = SettingsService.get("emby_api_key")
            ...
            logger.error(f"Failed to load Emby configuration: {e}")
            return MediaServerConfig(
                server_type=MediaServerType.EMBY, server_url="http://localhost:8096"
            )
    ...
    def authenticate(self) -> bool:
        ...
        try:
            if self.config.api_key:
                ...
                response = self._make_request("/emby/System/Info")
                if response.status_code == 200:
                    self._connected = True
                    logger.info("Emby API key authentication successful")
                    return True
            ...
            if self.config.username and self.config.password:
                ...
                response = self._make_request(
                    "/emby/Users/authenticatebyname", "POST", auth_data
                )
                ...
                if response.status_code == 200:
                    ...
                    if access_token:
                        ...
                        self._connected = True
                        logger.info(
                            f"Emby authentication successful for user: {user_info.get('Name')}"
                        )
                        return True

            logger.error("Emby authentication failed - no valid credentials")
            return False

        except Exception as e:
            logger.error(f"Emby authentication failed: {e}")
    ...
    def _make_request(
        ...
        """Make authenticated request to Emby server"""
        # Emby uses /emby prefix for API endpoints
        if not endpoint.startswith("/emby"):
            endpoint = f"/emby{endpoint}"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "product_name": server_info.get("ProductName", "Emby"),
    ...
    def get_server_info(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "product_name": system_info.get("ProductName", "Emby"),
                ...
                "system_architecture": system_info.get("SystemArchitecture"),
            }

        except Exception as e:
            logger.error(f"Failed to get Emby server info: {e}")
            return {}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in data.get("Items", []):
                    media_item = self._convert_emby_item_to_media_item(item)
                    if media_item:
    ...
    def get_play_history(self, user_id: str = None, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            # Emby has activity log endpoints that can be used for play history
            params = {
                "minDate": (datetime.now() - datetime.timedelta(days=30)).isoformat(),
                "limit": limit,
            }

            response = self._make_request("/System/ActivityLog/Entries", data=params)

            if response.status_code == 200:
                data = response.json()
                # Filter for playback activities
                play_activities = [
                    activity
                    for activity in data.get("Items", [])
                    if "played" in activity.get("Type", "").lower()
                    or "playback" in activity.get("Type", "").lower()
                ]
                return play_activities
            else:
                logger.error(f"Failed to get play history: HTTP {response.status_code}")
                return []
    ...
    def _convert_emby_item_to_media_item(self, item: Dict) -> Optional[MediaItem]:
        """Convert Emby item to unified MediaItem"""
        try:
            ...
            logger.error(f"Failed to convert Emby item: {e}")
            return None

    def _convert_search_hint_to_media_item(self, hint: Dict) -> Optional[MediaItem]:
        """Convert Emby search hint to MediaItem"""
        try:
