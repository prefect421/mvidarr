...
class CollectionMaintenanceService:
    ...
    async def analyze_storage(self) -> StorageAnalysis:
        ...
        try:
            ...
            video_size = await self._calculate_directory_size(f"{self.data_directory}/videos")
            thumbnail_size = await self._calculate_directory_size(f"{self.data_directory}/thumbnails")
            cache_size = await self._calculate_directory_size(f"{self.data_directory}/cache")
            ...
            logger.info(f"Storage analysis completed: {analysis.total_size_gb:.2f}GB total")
            return analysis
    ...
    async def detect_duplicates(
        self,
        strategy: DuplicateStrategy = None
    ) -> List[DuplicateGroup]:
        ...
        try:
            ...
            task = await self._create_maintenance_task(MaintenanceType.DUPLICATE_DETECTION)
            ...
            try:
                ...
                task.results = {
                    ...
                    'potential_duplicates': sum(len(group.videos) for group in duplicate_groups),
                    'potential_savings_gb': sum(group.potential_savings_bytes for group in duplicate_groups) / (1024**3)
                }
    ...
    async def cleanup_broken_links(self) -> CleanupResult:
        ...
        try:
            ...
            # Create maintenance task
            task = await self._create_maintenance_task(MaintenanceType.BROKEN_LINK_CLEANUP)
            ...
            try:
                    ...
                    for i, video in enumerate(videos):
                        try:
                            ...
                            if video.file_path and not os.path.exists(video.file_path):
                                ...
                                logger.info(f"Removed broken link: {video.title} ({video.file_path})")
                ...
                logger.info(f"Broken link cleanup completed: {items_cleaned} items cleaned")
                return result
    ...
    async def cleanup_orphaned_files(self) -> CleanupResult:
        ...
        try:
            ...
            try:
                ...
                async with get_async_session() as session:
                    videos_query = select(Video.file_path).where(Video.file_path.isnot(None))
                    result = await session.execute(videos_query)
                        ...
                        try:
                            if file_path not in db_file_paths:
                                ...
                                if file_age_hours > self.orphaned_file_age_hours:
                                    ...
                                    warnings.append(f"Recent orphaned file kept: {file_path}")
                ...
                logger.info(f"Orphaned files cleanup completed: {space_freed / (1024**2):.2f}MB freed")
                return result
    ...
    async def cleanup_cache_files(self) -> CleanupResult:
        ...
        try:
                    ...
                    for root, dirs, files in os.walk(cache_dir):
                        for file in files:
                            ...
                            try:
                                ...
                                errors.append(f"Error processing cache file {file_path}: {str(e)}")
            ...
            logger.info(f"Cache cleanup completed: {space_freed / (1024**2):.2f}MB freed")
            return result
    ...
    async def _find_largest_files(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            async with get_async_session() as session:
                query = select(Video.id, Video.title, Video.file_path, Video.file_size).where(
                    Video.file_size.isnot(None)
                ).order_by(desc(Video.file_size)).limit(limit)
    ...
    async def _find_oldest_files(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            async with get_async_session() as session:
                query = select(Video.id, Video.title, Video.file_path, Video.created_at).order_by(
                    asc(Video.created_at)
                ).limit(limit)
    ...
    async def _group_by_similar_titles(self, videos: List[Video]) -> Dict[str, List[Video]]:
        """Group videos by similar titles"""
        try:
    ...
    async def _analyze_duplicate_group(
        ...
        try:
            ...
            if strategy == DuplicateStrategy.KEEP_HIGHEST_QUALITY:
                ...
                recommended_keeper = max(videos, key=lambda v: v.created_at or datetime.min).id
            ...
            keeper_size = next((v.file_size or 0 for v in videos if v.id == recommended_keeper), 0)
    ...
    async def _create_maintenance_task(self, task_type: MaintenanceType) -> MaintenanceTask:
...
async def get_collection_maintenance_service(config: Optional[Dict] = None) -> CollectionMaintenanceService:
