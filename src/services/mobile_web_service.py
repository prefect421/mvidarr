...
class MobileWebService:
    ...
    async def get_mobile_dashboard(
        ...
        try:
            ...
            continue_watching = await self._get_continue_watching_mobile(user_id, limit=5)
            dashboard['continue_watching'] = continue_watching
            
            # Get favorite playlists
            favorite_playlists = await self._get_favorite_playlists_mobile(user_id, limit=3)
    ...
    async def browse_mobile_videos(
        ...
        try:
            ...
            for video in video_results.get('videos', []):
                mobile_card = await self._create_mobile_video_card(video, include_offline)
        ...
        try:
            ...
            if current_cache_size >= self.max_cache_size_mb:
                ...
                if current_cache_size >= self.max_cache_size_mb:
                    return {
                        'success': False,
                        'message': 'Insufficient cache space'
                    }
            
            # Get video details
            async with get_async_session() as session:
                video_query = select(Video).where(Video.id == video_id).options(selectinload(Video.artist))
    ...
    async def sync_offline_content(
        ...
        try:
            ...
            videos_to_cache = await self._get_videos_for_strategy(strategy, user_id, max_videos)
            ...
            for i in range(0, len(videos_to_cache), self.sync_batch_size):
                ...
                batch_tasks = [
                    self.cache_video_for_offline(video_id, MobileQuality.MEDIUM_720P, user_id)
                ...
                for result in batch_results:
                    if isinstance(result, Exception):
                        ...
                        if 'cache_item' in result:
                            sync_results['total_size_mb'] += result['cache_item'].get('cache_size_mb', 0)
                    else:
                        sync_results['videos_failed'] += 1
            ...
            logger.info(f"Offline sync completed: {sync_results['videos_cached']} cached, {sync_results['videos_failed']} failed")
    ...
    async def get_mobile_stats(self, user_id: Optional[int] = None) -> MobileStats:
        ...
        try:
            ...
            cache_hit_rate = (int(cache_hits) / total_requests * 100) if total_requests > 0 else 0
            ...
            sync_time = datetime.fromisoformat(last_sync) if last_sync else datetime.now()
            ...
            return MobileStats(
                ...
                bandwidth_saved_mb=round(total_cache_size * 0.8, 2)  # Estimate bandwidth savings
            )
    ...
    async def clear_offline_cache(
        ...
        try:
            ...
            logger.info(f"Cleared {cleared_count} cached videos, freed {freed_space_mb:.2f}MB")
    ...
    async def _get_recent_mobile_videos(self, limit: int = 10) -> List[MobileVideoCard]:
        ...
        try:
            async with get_async_session() as session:
                query = select(Video).options(selectinload(Video.artist)).order_by(desc(Video.created_at)).limit(limit)
    ...
    async def _cache_video_file(self, video: Video, quality: MobileQuality) -> Dict[str, Any]:
    ...
    async def _save_cache_item(self, cache_item: OfflineCacheItem):
        ...
        try:
            ...
            cache_data = {
                ...
                'expires_at': cache_item.expires_at.isoformat() if cache_item.expires_at else None
            }
            
            await self.redis_client.setex(cache_key, 86400 * self.cache_retention_days, json.dumps(cache_data))
        ...
        try:
            ...
            async with get_async_session() as session:
                if strategy == OfflineStrategy.RECENT_WATCHED:
                    ...
                    query = select(Video.id).order_by(desc(Video.last_watched)).limit(max_videos)
                    result = await session.execute(query)
                    ...
                    query = select(Video.id).where(Video.is_music_video == True).limit(max_videos)
                    result = await session.execute(query)
                    video_ids = [row[0] for row in result.all()]
                    ...
                    ).order_by(desc(Video.view_count), desc(Video.created_at)).limit(max_videos)
                    result = await session.execute(query)
                    video_ids = [row[0] for row in result.all()]
                
                else:
                    # Default to recent videos
                    query = select(Video.id).order_by(desc(Video.created_at)).limit(max_videos)
                    result = await session.execute(query)
                    video_ids = [row[0] for row in result.all()]
    ...
    async def _get_continue_watching_mobile(self, user_id: Optional[int], limit: int) -> List[Dict]:
        """Get continue watching for mobile"""
        # Placeholder - would integrate with watch history service
        return []
    
    async def _get_favorite_playlists_mobile(self, user_id: Optional[int], limit: int) -> List[Dict]:
    ...
    async def _get_mobile_quick_actions(self, user_id: Optional[int]) -> List[Dict[str, str]]:
    ...
    async def _update_sync_stats(self, sync_results: Dict[str, Any]):
        """Update sync statistics"""
        try:
            await self.redis_client.setex("last_offline_sync", 86400, datetime.now().isoformat())
            await self.redis_client.setex("last_sync_results", 86400, json.dumps(sync_results))
