...
class CrossPlatformVideo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'quality_scores': {p.value: score for p, score in self.quality_scores.items()},
...
class VideoIntelligenceResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'cross_platform_matches': [v.to_dict() for v in self.cross_platform_matches],
...
class CrossPlatformVideoIntelligence:
    ...
    async def correlate_videos_across_platforms(
        ...
        try:
            if not platforms:
                platforms = [Platform.VEVO, Platform.YOUTUBE, Platform.VIMEO, Platform.IMVDB]
            ...
            platform_results = await self._search_all_platforms(artist, title, platforms)
            ...
            best_recommendations = await self._generate_best_recommendations(cross_platform_matches)
            ...
            # Track performance
            await track_media_processing_time("cross_platform_video_intelligence", processing_time)
        ...
        try:
            ...
            for i, (platform, _) in enumerate(search_tasks):
                ...
                if isinstance(result, Exception):
                    ...
                    platform_results[platform] = result[:self.config['max_videos_per_platform']]
    ...
    async def _search_imvdb(self, artist: str, title: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            results = await asyncio.to_thread(imvdb_service.search_videos, artist, title)
    ...
    async def _correlate_platform_results(
        ...
        try:
            ...
            cross_platform_videos.sort(
                key=lambda v: (len(v.platforms), v.match_confidence),
                reverse=True
            )
    ...
    async def _videos_match(self, video1: Dict[str, Any], video2: Dict[str, Any]) -> bool:
    ...
    async def _analyze_cross_platform_video(self, cross_platform_video: CrossPlatformVideo):
            ...
            for match in cross_platform_matches[:10]:  # Top 10 matches
                if match.best_version:
                    ...
                    recommendation.update({
                        ...
                        'recommendation_score': match.match_confidence * len(match.platforms)
                    })
    ...
    async def track_video_availability_changes(
        ...
        try:
            ...
            current_analysis = await self.correlate_videos_across_platforms(artist, title)
            
            availability = {
                'artist': artist,
                'title': title,
                'platforms_available': [p.value for p in Platform if any(
                    p in match.platforms for match in current_analysis.cross_platform_matches
                )],
                'total_versions': len(current_analysis.cross_platform_matches),
                'last_checked': time.time(),
                'availability_score': len(current_analysis.cross_platform_matches) / len(Platform)
            }
    ...
    async def generate_video_discovery_insights(
        self,
        artist: str
    ) -> Dict[str, Any]:
    ...
    async def get_intelligence_statistics(self) -> Dict[str, Any]:
        ...
        try:
            avg_processing_time = (
                self.stats['total_processing_time'] / max(1, self.stats['analyses_completed'])
            )
            
            return {
                "service": "Cross-Platform Video Intelligence",
                "analyses_completed": self.stats['analyses_completed'],
                "cross_platform_matches_found": self.stats['cross_platform_matches_found'],
