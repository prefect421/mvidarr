...
class PersonalBackupService:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.rate_limits = {
            CloudProvider.GOOGLE_DRIVE: {'requests_per_minute': 100, 'bytes_per_second': 10 * 1024 * 1024},  # 10MB/s
            CloudProvider.DROPBOX: {'requests_per_minute': 60, 'bytes_per_second': 8 * 1024 * 1024},         # 8MB/s  
            CloudProvider.ONEDRIVE: {'requests_per_minute': 80, 'bytes_per_second': 12 * 1024 * 1024}       # 12MB/s
        }
        ...
        try:
            ...
            if provider == CloudProvider.GOOGLE_DRIVE:
                ...
                if cloud_creds.access_token:
                    is_valid = await self._validate_google_drive_credentials(cloud_creds)
                    if not is_valid:
    ...
    async def create_backup_job(
        ...
        try:
            ...
            if provider not in self.credentials or not self.credentials[provider].is_valid():
            ...
            job.destination_path = destination_path or f"MVidarr Backup/{backup_type.value}"
            ...
            logger.info(f"Created backup job {job_id}: {total_files} files, {total_size / (1024*1024):.1f} MB")
    ...
    async def start_backup_job(self, job_id: str, progress_callback: Optional[Callable] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Backup job {job_id} {'completed' if result['success'] else 'failed'}")
    ...
    async def _execute_google_drive_backup(self, job: BackupJob, progress_callback: Optional[Callable]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not credentials.is_valid():
                return {'success': False, 'error': 'Invalid or expired Google Drive credentials'}
            
            uploaded_files = []
            
            # Create folder in Google Drive
            folder_id = await self._create_google_drive_folder(credentials, job.destination_path)
                    ...
                    if os.path.exists(file_path):
                        ...
                        if file_id:
                            ...
                            if progress_callback:
                                await progress_callback(job.job_id, job.progress_percent, os.path.basename(file_path))
    ...
    async def _execute_dropbox_backup(self, job: BackupJob, progress_callback: Optional[Callable]) -> Dict[str, Any]:
                ...
                try:
                    if os.path.exists(file_path):
                        ...
                        if success:
                            ...
                            # Rate limiting
                            await asyncio.sleep(1.0)  # Conservative rate limiting for Dropbox
    ...
    async def _execute_onedrive_backup(self, job: BackupJob, progress_callback: Optional[Callable]) -> Dict[str, Any]:
    ...
    async def _validate_google_drive_credentials(self, credentials: CloudCredentials) -> bool:
        """Validate Google Drive credentials"""
    ...
    async def _validate_dropbox_credentials(self, credentials: CloudCredentials) -> bool:
        """Validate Dropbox credentials"""
        # This would make a test API call to Dropbox
        return bool(credentials.access_token)
    
    async def _validate_onedrive_credentials(self, credentials: CloudCredentials) -> bool:
        """Validate OneDrive credentials"""
        # This would make a test API call to OneDrive
        return bool(credentials.access_token)
    
    async def _create_google_drive_folder(self, credentials: CloudCredentials, folder_name: str) -> str:
    ...
    async def _upload_file_to_google_drive(self, credentials: CloudCredentials, file_path: str, folder_id: str) -> Optional[str]:
    ...
    async def _upload_file_to_dropbox(self, credentials: CloudCredentials, file_path: str, destination_path: str) -> bool:
        """Upload file to Dropbox"""
        # This would implement the actual Dropbox file upload
        await asyncio.sleep(0.1)  # Simulate upload time
        return True
    
    async def _upload_file_to_onedrive(self, credentials: CloudCredentials, file_path: str, destination_path: str) -> Optional[str]:
        """Upload file to OneDrive"""
        # This would implement the actual OneDrive file upload
        await asyncio.sleep(0.1)  # Simulate upload time
        return f"file_{hashlib.md5(file_path.encode()).hexdigest()[:12]}"
    
    async def _save_credentials(self, provider: CloudProvider, credentials: CloudCredentials):
        """Save credentials to secure storage"""
        try:
            # Store in Redis with encryption (in production, use proper encryption)
            cache_key = f"cloud_credentials:{provider.value}"
            await self.redis_client.setex(cache_key, 86400 * 30, json.dumps(credentials.to_dict()))
            ...
            for provider in CloudProvider:
                ...
                if cached_creds:
                    ...
                    if creds_data.get('expires_at'):
                        credentials.expires_at = datetime.fromisoformat(creds_data['expires_at'])
...
async def get_personal_backup_service(config: Optional[Dict] = None) -> PersonalBackupService:
