...
from src.services.analytics_service import get_analytics_service, MetricPoint, MetricType
...
class RequestContext:
    ...
    span_id: str = field(default_factory=lambda: hashlib.md5(str(random.random()).encode()).hexdigest()[:8])
...
class ServiceRegistry:
    ...
    async def register_service(self, instance: ServiceInstance) -> bool:
        ...
        try:
            ...
            logger.info(f"📝 Service registered: {instance.service_name} ({instance.service_id})")
            return True
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.debug(f"✅ Health check passed: {instance.service_id} ({response_time:.1f}ms)")
                    else:
                        instance.status = ServiceStatus.DEGRADED
                        instance.consecutive_failures += 1
                        logger.warning(f"⚠️ Health check degraded: {instance.service_id} - {response.status}")
        
        except asyncio.TimeoutError:
            instance.consecutive_failures += 1
            instance.status = ServiceStatus.UNHEALTHY if instance.consecutive_failures >= 3 else ServiceStatus.DEGRADED
...
class LoadBalancer:
    ...
    def __init__(self, algorithm: LoadBalancerAlgorithm = LoadBalancerAlgorithm.ROUND_ROBIN):
        ...
        self.consistent_hash_ring: Dict[str, Dict[int, ServiceInstance]] = defaultdict(dict)
    ...
    def _consistent_hash_select(self, instances: List[ServiceInstance], 
                               context: RequestContext) -> ServiceInstance:
...
class APIGateway:
    ...
    async def add_routing_rule(self, rule: RouteRule):
        ...
        logger.info(f"➕ Routing rule added: {rule.path_pattern} -> {rule.target_service}")
    ...
    async def process_request(self, path: str, method: str, headers: Dict[str, str],
                            body: bytes, query_params: Dict[str, str],
                            client_ip: str) -> Tuple[int, Dict[str, str], bytes]:
        ...
        try:
            ...
            return 500, {"Content-Type": "application/json"}, b'{"error": "Internal gateway error"}'
    ...
    async def _forward_request(self, context: RequestContext, rule: RouteRule,
                             path: str, method: str, headers: Dict[str, str],
                             body: bytes, query_params: Dict[str, str]) -> Tuple[int, Dict[str, str], bytes]:
        ...
        try:
            ...
            status_code, response_headers, response_body = await self._make_request_with_retries(
    ...
    async def _make_request_with_retries(self, url: str, method: str, headers: Dict[str, str],
                                       body: bytes, query_params: Dict[str, str],
                                       rule: RouteRule) -> Tuple[int, Dict[str, str], bytes]:
        ...
        return 502, {"Content-Type": "application/json"}, b'{"error": "Service request failed"}'
    
    def _extract_api_version(self, headers: Dict[str, str], 
                            query_params: Dict[str, str], path: str) -> str:
    ...
    async def _record_request_metrics(self, context: RequestContext, status_code: int, result: str):
        """Record request metrics for analytics"""
        try:
            analytics_service = await get_analytics_service()
            
            # Record gateway metrics
            await analytics_service.record_metric(MetricPoint(
    ...
    async def get_gateway_stats(self) -> Dict[str, Any]:
        ...
        try:
            total_services = sum(len(instances) for instances in self.service_registry.services.values())
