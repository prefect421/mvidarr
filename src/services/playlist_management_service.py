...
class PlaylistManagementService:
        ...
        self.default_smart_playlists = [
            ...
            },
            {
                ...
                "rules": [
                    ...
                    PlaylistRule(
                        SmartPlaylistRule.VIEW_COUNT_GREATER_THAN, "greater_than", 0
                    ),
                ],
            },
            {
            ...
            },
            {
                ...
                "rules": [PlaylistRule(SmartPlaylistRule.QUALITY_IS, "equals", "720p")],
            },
        ]
    ...
    async def create_playlist(
        ...
        try:
            ...
            logger.info(
                f"Created playlist '{name}' ({playlist_id}) with {len(playlist.items)} items"
            )
    ...
    async def add_videos_to_playlist(
        ...
        try:
            ...
            if len(playlist.items) + len(video_ids) > self.max_playlist_size:
                raise ValueError(
                    f"Playlist size limit ({self.max_playlist_size}) would be exceeded"
                )
            ...
            async with get_async_session() as session:
                ...
                current_max_position = max(
                    [item.position for item in playlist.items], default=0
                )
    ...
    async def remove_videos_from_playlist(
        self, playlist_id: str, video_ids: List[int], user_id: Optional[int] = None
    ) -> Playlist:
        ...
        try:
            ...
            if playlist.playlist_type != PlaylistType.MANUAL:
                raise ValueError(
                    "Cannot manually remove videos from non-manual playlists"
                )

            # Remove videos
            original_count = len(playlist.items)
            playlist.items = [
                item for item in playlist.items if item.video_id not in video_ids
            ]
    ...
    async def reorder_playlist(
        ...
        try:
            ...
            logger.info(
                f"Reordered video {video_id} to position {new_position} in playlist {playlist_id}"
            )
    ...
    async def update_playlist(
        self, playlist_id: str, updates: Dict[str, Any], user_id: Optional[int] = None
    ) -> Playlist:
    ...
    async def delete_playlist(
        self, playlist_id: str, user_id: Optional[int] = None
    ) -> bool:
        """Delete a playlist"""
        try:
            playlist = await self._load_playlist(playlist_id)
    ...
    async def list_playlists(
        ...
        try:
            ...
            if playlist_type:
                filtered_playlists = [
                    p for p in all_playlists if p.playlist_type == playlist_type
                ]
            else:
            ...
            recent = [
                p for p in all_playlists if p.playlist_type == PlaylistType.MANUAL
            ][:5]
            popular = sorted(
                [p for p in all_playlists], key=lambda p: p.play_count, reverse=True
            )[:5]
    ...
    async def update_smart_playlists(self, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            smart_playlists = [
                ...
                if p.playlist_type in self.smart_playlist_update_intervals
            ]
            ...
            for playlist in smart_playlists:
                try:
                    ...
                    if not force:
                        update_interval = self.smart_playlist_update_intervals.get(
                            playlist.playlist_type, 86400
                        )
                        time_since_update = (
                            datetime.now() - playlist.stats.last_updated
                        ).total_seconds()
                    ...
                    logger.error(
                        f"Failed to update smart playlist {playlist.playlist_id}: {e}"
                    )
                    error_count += 1

            logger.info(
                f"Updated {updated_count} smart playlists, {error_count} errors"
            )
    ...
    async def generate_discovery_playlist(
        ...
        try:
            async with get_async_session() as session:
                ...
                if video_ids:
                    await self.add_videos_to_playlist(
                        playlist.playlist_id, video_ids, user_id
                    )

                logger.info(
                    f"Generated discovery playlist with {len(video_ids)} videos"
                )
    ...
    async def _update_smart_playlist(self, playlist: Playlist):
        ...
        try:
            async with get_async_session() as session:
                ...
                base_query = base_query.order_by(desc(Video.created_at)).limit(
                ...
                for i, video in enumerate(videos):
                    playlist_item = PlaylistItem(
                        video_id=video.id, position=i + 1, added_at=datetime.now()
                    )
                    playlist.items.append(playlist_item)
                ...
                logger.info(
                    f"Updated smart playlist {playlist.playlist_id} with {len(videos)} videos"
                )
    ...
    async def _update_playlist_stats(self, playlist: Playlist, session: AsyncSession):
        ...
        try:
            ...
            most_played_video = (
                max(videos, key=lambda v: v.view_count or 0) if videos else None
            )
            ...
            playlist.stats = PlaylistStats(
                ...
                most_played_video_id=(
                    most_played_video.id if most_played_video else None
                ),
    ...
    async def _save_playlist(self, playlist: Playlist):
        ...
        try:
            ...
            await self.redis_client.setex(
                cache_key, 86400 * 7, json.dumps(playlist_data)
            )
    ...
    async def _create_default_playlists(self):
        ...
        try:
            ...
            logger.info(
                f"Ensured {len(self.default_smart_playlists)} default playlists exist"
            )
...
async def get_playlist_management_service(
