...
from src.database import get_async_session, get_engine
from src.models.video import Video
from src.models.artist import Artist
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select, func, and_, or_, desc, asc, text, Index, event
...
class CollectionPerformanceService:
    ...
    async def analyze_collection_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                'optimization_history': [opt.__dict__ for opt in self.applied_optimizations[-5:]],
                'analysis_timestamp': datetime.now().isoformat()
            }
            
            logger.info(f"Collection performance analysis completed: {len(bottlenecks)} bottlenecks identified")
            return analysis
    ...
    async def optimize_for_large_collection(
        ...
        try:
            ...
            overall_improvement = await self._calculate_improvement(before_metrics, after_metrics)
            ...
            logger.info(f"Large collection optimization completed with {overall_improvement:.1f}% improvement")
            return result
    ...
    async def optimize_query_performance(
        ...
        try:
            query_patterns = query_patterns or [
                'video_search', 'artist_browse', 'playlist_load',
                'thumbnail_fetch', 'metadata_update'
            ]
    ...
    async def optimize_memory_footprint(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'memory_reduction_percent': (memory_saved_mb / current_memory) * 100 if current_memory > 0 else 0,
    ...
    async def get_performance_recommendations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            slow_queries = [q for q, time in current_metrics.query_response_times.items() 
            ...
            low_cache_hits = [cache for cache, rate in current_metrics.cache_hit_rates.items() 
                            if rate < 70.0]
            if low_cache_hits:
                recommendations.append({
                    'type': 'caching',
    ...
    async def _get_collection_stats(self) -> CollectionStats:
        ...
        try:
            async with get_async_session() as session:
                ...
                # Size statistics
                size_query = select(func.sum(Video.file_size), func.avg(Video.file_size))
                ...
                # Date range
                date_query = select(func.min(Video.created_at), func.max(Video.created_at))
                ...
                quality_query = select(Video.quality, func.count(Video.id)).group_by(Video.quality)
                quality_result = await session.execute(quality_query)
                quality_dist = {row[0] or 'unknown': row[1] for row in quality_result.all()}
            ...
            return CollectionStats(0, 0, 0, 0, datetime.now(), datetime.now(), [], {}, {}, {})
    ...
    async def _identify_bottlenecks(self, metrics: PerformanceMetrics) -> List[Dict[str, Any]]:
    ...
    async def _optimize_database_for_scale(self, level: OptimizationLevel) -> OptimizationResult:
    ...
    async def _optimize_caching_for_scale(self, level: OptimizationLevel) -> OptimizationResult:
    ...
    async def _optimize_queries_for_scale(self, level: OptimizationLevel) -> OptimizationResult:
    ...
    async def _calculate_improvement(
        ...
        try:
            ...
            memory_improvement = max(0, (before.memory_usage_mb - after.memory_usage_mb) / before.memory_usage_mb * 100)
            cpu_improvement = max(0, (before.cpu_usage_percent - after.cpu_usage_percent) / before.cpu_usage_percent * 100)
    ...
    async def _configure_intelligent_cache(self, strategy: CacheStrategy) -> Dict[str, Any]:
    ...
    async def _configure_cache_eviction(self, strategy: CacheStrategy) -> Dict[str, Any]:
        """Configure cache eviction policies"""
        return {
...
async def get_collection_performance_service(config: Optional[Dict] = None) -> CollectionPerformanceService:
