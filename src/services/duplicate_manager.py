...
class DuplicateType(Enum):
    ...
    VERSION_VARIANT = "version_variant"           # Same song, different version (remix, live, etc.)
...
class DuplicateGroup:
    ...
    def add_file(self, file_path: str, similarity_score: float = 0.0, metadata: Optional[Dict] = None):
...
class DuplicateScanResult:
    ...
    def get_groups_by_confidence(self, min_confidence: DuplicateConfidence) -> List[DuplicateGroup]:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            'summary': {
                'very_high_confidence': len(self.get_groups_by_confidence(DuplicateConfidence.VERY_HIGH)),
                'high_confidence': len(self.get_groups_by_confidence(DuplicateConfidence.HIGH)),
                'medium_confidence': len(self.get_groups_by_confidence(DuplicateConfidence.MEDIUM)),
                'low_confidence': len(self.get_groups_by_confidence(DuplicateConfidence.LOW))
            }
        }
...
class DuplicateManager:
    ...
    async def scan_for_duplicates(
        ...
        try:
            ...
            logger.info(f"Starting duplicate scan in {directory} (min_confidence={min_confidence.value})")
            ...
            if len(video_files) < 2:
                result.warnings.append(f"Need at least 2 videos for duplicate detection. Found {len(video_files)}")
                return result
            ...
            if len(music_video_files) < len(video_files):
                logger.info(f"Filtering to {len(music_video_files)} music videos out of {len(video_files)} total files")
            ...
            if len(remaining_files) >= 2:
                logger.info(f"Analyzing {len(remaining_files)} files for similarity duplicates")
            ...
            logger.info(f"Duplicate scan completed: {result.duplicate_groups_found} groups found, "
    ...
    async def remove_duplicates(
        ...
        try:
            logger.info(f"Removing duplicates: {len(duplicate_groups)} groups "
            ...
            for i, group in enumerate(duplicate_groups):
                try:
                    ...
                    if not keep_file:
                        removal_result['warnings'].append(f"Could not determine file to keep in group {group.group_id}")
                        continue
                    ...
                    error_msg = f"Failed to process duplicate group {group.group_id}: {e}"
                    removal_result['errors'].append(error_msg)
                    logger.error(error_msg)
            
            logger.info(f"Duplicate removal completed: {removal_result['files_removed']} files removed, "
    ...
    async def _calculate_file_hashes(self, video_files: List[str], progress_callback=None):
        """Calculate file hashes for exact duplicate detection"""
        self.file_hashes = {}
        ...
        for i, video_path in enumerate(video_files):
            try:
                ...
                if progress_callback and i % 10 == 0:  # Update every 10 files
                    await progress_callback(i, len(video_files), f"Hashing {os.path.basename(video_path)}")
        ...
        try:
            ...
            for i, video_path in enumerate(video_files):
                try:
                    ...
                    fingerprint = await self.fingerprinting_service.generate_fingerprint(video_path)
                    fingerprints[video_path] = fingerprint
                    
                    if progress_callback and i % 5 == 0:
                        await progress_callback(i, len(video_files), f"Analyzing {os.path.basename(video_path)}")
                        
                except Exception as e:
                    logger.warning(f"Failed to generate fingerprint for {video_path}: {e}")
            ...
            for i, file1 in enumerate(file_paths):
                ...
                if len(similar_files) > 1:
                    ...
                    group.duplicate_type = self._determine_duplicate_type(similar_files, similarity_scores)
                    ...
                    group.detection_factors = {
                        ...
                        'similarity_range': [min(similarity_scores.values()), max(similarity_scores.values())]
                    }
    ...
    async def _calculate_similarity(self, fingerprint1: Any, fingerprint2: Any) -> float:
        ...
        try:
            ...
            similarity = await self.fingerprinting_service.calculate_similarity(fingerprint1, fingerprint2)
            return similarity
        except Exception as e:
    ...
    def _determine_duplicate_type(self, files: List[str], similarity_scores: Dict[str, float]) -> DuplicateType:
        ...
        version_keywords = ['remix', 'live', 'acoustic', 'cover', 'radio edit', 'extended']
    ...
    async def _select_highest_quality_file(self, files: List[str]) -> str:
        ...
        try:
            ...
            try:
                return max(files, key=lambda x: os.path.getsize(x) if os.path.exists(x) else 0)
            except:
    ...
    async def _determine_file_to_keep(self, group: DuplicateGroup, strategy: str) -> Optional[str]:
        ...
        if strategy == "highest_quality":
            ...
            try:
                return max(group.duplicate_files, 
            ...
            try:
                return min(group.duplicate_files,
                          key=lambda x: os.path.getsize(x) if os.path.exists(x) else float('inf'))
            except:
                return group.duplicate_files[0]
