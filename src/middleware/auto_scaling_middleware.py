...
class AdaptiveResourceManager:
    ...
    def _update_adaptive_thresholds(self):
        ...
        logger.debug(f"🔧 Updated adaptive thresholds: CPU={self.thresholds['cpu_high']:.1f}%, Memory={self.thresholds['memory_high']:.1f}%")
    ...
    def assess_load_level(self) -> LoadLevel:
        ...
        if (latest.cpu_percent >= self.thresholds["cpu_critical"] or
            ...
            latest.avg_response_time_ms >= self.thresholds["response_time_critical"] or
            latest.error_rate_percent >= self.thresholds["error_rate_critical"]):
            return LoadLevel.CRITICAL
        
        # High load conditions
        if (latest.cpu_percent >= self.thresholds["cpu_high"] or
            latest.memory_percent >= self.thresholds["memory_high"] or
            latest.avg_response_time_ms >= self.thresholds["response_time_high"] or
            latest.error_rate_percent >= self.thresholds["error_rate_high"]):
            return LoadLevel.HIGH
        
        # Low load conditions
        if (latest.cpu_percent <= self.baselines["normal_cpu"] * 0.5 and
            latest.memory_percent <= self.baselines["normal_memory"] * 0.7 and
            latest.avg_response_time_ms <= self.baselines["normal_response_time"] * 0.8):
    ...
    def make_scaling_decision(self) -> Optional[ScalingDecision]:
        ...
        if load_level == LoadLevel.CRITICAL:
            ...
            recent_low_count = sum(1 for m in list(self.metrics_history)[-30:] 
        ...
        elif load_level == LoadLevel.HIGH and latest_metrics.avg_response_time_ms > self.baselines["normal_response_time"] * 3:
            decision = ScalingDecision(
    ...
    def _analyze_trends(self) -> Dict[str, Any]:
        ...
        older_samples = list(self.metrics_history)[-20:-10] if len(self.metrics_history) >= 20 else recent_samples
        ...
        recent_memory = sum(m.memory_percent for m in recent_samples) / len(recent_samples)
        older_memory = sum(m.memory_percent for m in older_samples) / len(older_samples)
        
        recent_response = sum(m.avg_response_time_ms for m in recent_samples) / len(recent_samples)
        older_response = sum(m.avg_response_time_ms for m in older_samples) / len(older_samples)
...
class AutoScalingMiddleware(BaseHTTPMiddleware):
    ...
    async def _collect_metrics(self) -> ResourceMetrics:
        ...
        try:
            ...
            requests_per_second = self.total_requests / 60 if self.total_requests > 0 else 0
    ...
    async def _scale_up_resources(self, decision: ScalingDecision):
        ...
        try:
            await self.cache_manager.set(
                f"scaling_event_{int(time.time())}", 
                str(scaling_event), 
                ttl=86400
            )
        except Exception as e:
