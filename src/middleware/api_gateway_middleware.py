...
from src.services.api_gateway import get_api_gateway, RouteRule, ServiceInstance, RoutingStrategy
...
class APIGatewayMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, gateway_enabled: bool = True, gateway_prefix: str = "/gateway"):
        super().__init__(app)
        ...
        logger.info(f"🚪 API Gateway middleware initialized (enabled: {gateway_enabled})")
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not self.gateway_enabled or not request.url.path.startswith(self.gateway_prefix):
    ...
    async def _process_gateway_request(self, request: Request) -> Response:
        ...
        try:
            ...
            status_code, response_headers, response_body = await gateway.process_request(
...
class GatewayManagementMiddleware(BaseHTTPMiddleware):
    ...
    async def _process_management_request(self, request: Request) -> Response:
        ...
        try:
            ...
            # Health check
                ...
                return JSONResponse(
                    status_code=404,
                    content={"error": "Management endpoint not found"}
                )
    ...
    async def _register_service(self, request: Request, gateway) -> Response:
        ...
        try:
            ...
            if success:
                ...
                return JSONResponse(
                    status_code=400,
                    content={"error": "Service registration failed"}
                )
                
        except Exception as e:
            return JSONResponse(
                status_code=400,
                content={"error": "Invalid service registration data", "details": str(e)}
            )
    ...
    async def _deregister_service(self, request: Request, gateway) -> Response:
        ...
        try:
            ...
            if not service_id:
                return JSONResponse(
                    status_code=400,
                    content={"error": "service_id required"}
                )
    ...
    async def _list_services(self, gateway) -> Response:
        ...
        try:
            ...
            return JSONResponse({
                "services": services,
                "total_services": sum(len(instances) for instances in gateway.service_registry.services.values())
            })
    ...
    async def _add_route(self, request: Request, gateway) -> Response:
        ...
        try:
            ...
            route = RouteRule(
                ...
                routing_strategy=RoutingStrategy(data.get("routing_strategy", "round_robin")),
