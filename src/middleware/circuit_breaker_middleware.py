...
class CircuitBreaker:
    ...
    def _calculate_avg_response_time(self) -> float:
        ...
        return sum(m.response_time_ms for m in self.request_metrics) / len(
    ...
    def _should_open_circuit(self) -> bool:
        ...
        if (
            ...
            and len(self.request_metrics) >= self.config.min_requests_for_stats
        ):
            return True
    ...
    def _transition_to_state(self, new_state: CircuitState, reason: str = ""):
        ...
        logger.info(
            f"🔌 Circuit '{self.name}' transitioned from {old_state.value} to {new_state.value}: {reason}"
        )
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            if self.success_count >= self.config.recovery_threshold:
                self._transition_to_state(
                    CircuitState.CLOSED,
                    f"Recovery successful ({self.success_count} successes)",
                )
    ...
    def record_failure(
        ...
        if self.state == CircuitState.CLOSED:
            ...
            self.half_open_requests += 1
            self._transition_to_state(
                CircuitState.OPEN, "Failed during recovery attempt"
            )
        ...
        for metric in list(self.request_metrics)[-5:]:
            if not metric.success and metric.error_message:
                recent_failures.append(
                    f"{metric.failure_type.value}: {metric.error_message}"
                )
...
class CircuitBreakerManager:
    ...
    def get_fallback_response(
        self, endpoint: str, error_details: str
    ) -> Dict[str, Any]:
...
class CircuitBreakerMiddleware(BaseHTTPMiddleware):
    ...
    def _determine_failure_type(
        ...
        if response_time_ms > self.circuit_manager.default_config.timeout_duration_ms:
            ...
            response_time_ms
            > self.circuit_manager.default_config.slow_request_threshold_ms
        ):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not circuit_breaker.can_execute():
            logger.warning(
                f"🔌 Circuit breaker OPEN for service '{service_name}' - request blocked"
            )
            ...
            fallback_response = self.circuit_manager.get_fallback_response(
                request.url.path, f"Service '{service_name}' circuit breaker is open"
            )
            ...
            return JSONResponse(
                ...
                headers={
                    ...
                    "Retry-After": str(
                        self.circuit_manager.default_config.circuit_open_duration_ms
                        // 1000
                    ),
                },
            )
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        await self.circuit_manager.cache_manager.set(
                            cache_key, "success_response_cached", ttl=3600
                        )
                    except Exception as e:
