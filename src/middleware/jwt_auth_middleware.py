...
class JWTManager:
    ...
    def create_access_token(self, user_claims: UserClaims, request: Optional[Request] = None) -> TokenData:
    ...
    def create_refresh_token(self, user_claims: UserClaims, request: Optional[Request] = None) -> TokenData:
    ...
    def verify_token(self, token: str, token_type: TokenType, request: Optional[Request] = None) -> Tuple[bool, Optional[UserClaims], Optional[str]]:
        """
    ...
    def refresh_access_token(self, refresh_token: str, request: Optional[Request] = None) -> Tuple[bool, Optional[TokenData], Optional[str]]:
        """
        Refresh access token using refresh token
        Returns: (success, new_token_data, error_message)
        """
        # Verify refresh token
        is_valid, user_claims, error = self.verify_token(refresh_token, TokenType.REFRESH, request)
        ...
        if self.blacklist.get_failed_attempts(user_id) >= self.config.max_refresh_attempts:
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if required_role and not self._has_permission(user_claims, required_role):
            return JSONResponse(
                ...
                content={"detail": f"Insufficient permissions. Required: {required_role.value}"}
            )
