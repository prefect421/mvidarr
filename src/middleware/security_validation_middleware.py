...
class SecurityValidator:
    ...
    def __init__(self, config: SecurityValidationConfig):
        ...
        self.xss_patterns = [re.compile(pattern, re.IGNORECASE | re.DOTALL) for pattern in config.XSS_PATTERNS]
    ...
    def validate_json_structure(self, data: Dict[str, Any], max_depth: int = 10) -> bool:
...
class SecurityValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            logger.debug(f"🛡️ Security validation passed for {request.url.path} in {processing_time:.3f}s")
            
            return response
            
        except HTTPException as e:
            # Log security violation
            logger.warning(f"🚨 Security validation failed: {e.detail} for {request.client.host}:{request.url.path}")
            return JSONResponse(
    ...
    async def _check_rate_limiting(self, request: Request):
        ...
        client_data['minute'] = {k: v for k, v in client_data['minute'].items() if k >= current_minute - 1}
        client_data['hour'] = {k: v for k, v in client_data['hour'].items() if k >= current_hour - 1}
    ...
    async def _validate_url_path(self, request: Request):
        ...
        for param_value in request.query_params.values():
            if (self.validator.detect_sql_injection(param_value) or 
                self.validator.detect_xss(param_value) or 
                self.validator.detect_command_injection(param_value)):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
    ...
    async def _validate_json_values(self, data: Any, path: str = ""):
        ...
        if isinstance(data, dict):
            ...
            if (self.validator.detect_sql_injection(data) or 
                self.validator.detect_xss(data) or 
                self.validator.detect_command_injection(data)):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
