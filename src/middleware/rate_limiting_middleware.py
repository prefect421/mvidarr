...
class AdaptiveRateLimiter:
    ...
    async def _cleanup_loop(self):
        ...
        while True:
            try:
                ...
                for client_ip, metrics in self.client_metrics.items():
                    ...
                    if current_time - metrics.last_seen > 3600:
                        ...
                        while (
                            ...
                            and current_time - metrics.request_times[0] > 3600
                        ):
                ...
                logger.debug(
                    f"🧹 Cleaned up {len(expired_clients)} expired client metrics"
                )
    ...
    async def _assess_threat_level(
        self, request: Request, client_metrics: ClientMetrics
    ) -> ThreatLevel:
        ...
        recent_requests = [
            t for t in client_metrics.request_times if current_time - t <= 60
        ]
        if len(recent_requests) > 0:
            requests_per_second = len(recent_requests) / min(
                60, current_time - recent_requests[0]
            )
            if (
                requests_per_second
                > self.config.ddos_detection["rapid_requests_threshold"]
            ):
                threat_score += 40
        ...
        if (
            len(client_metrics.endpoint_counts)
            > self.config.ddos_detection["unique_endpoint_threshold"]
        ):
            threat_score += 25

        # Check suspicious patterns
        if (
            len(client_metrics.suspicious_patterns)
            >= self.config.ddos_detection["suspicious_patterns_threshold"]
        ):
            threat_score += 35
        ...
        for pattern in self.config.suspicious_patterns:
            if re.search(pattern, path + query_string, re.IGNORECASE):
                if pattern not in client_metrics.suspicious_patterns:
                    ...
                    logger.warning(
                        f"🚨 Suspicious pattern detected: {pattern} from {client_metrics.ip_address}"
                    )
    ...
    async def check_rate_limit(
        ...
        if client_metrics.block_until and current_time < client_metrics.block_until:
            ...
            return (
                False,
                f"Temporarily blocked due to rate limit violation",
                remaining_time,
            )
        ...
        client_metrics.endpoint_counts[path] = (
            client_metrics.endpoint_counts.get(path, 0) + 1
        )
        client_metrics.method_counts[method] = (
            client_metrics.method_counts.get(method, 0) + 1
        )
        ...
        # Assess threat level
        client_metrics.threat_level = await self._assess_threat_level(
            request, client_metrics
        )
        ...
        if len(second_requests) > max_per_second:
            ...
            return (
                False,
                f"Rate limit exceeded: {len(second_requests)} requests per second",
                rule.block_duration,
            )
        ...
        if len(minute_requests) > max_per_minute:
            ...
            return (
                False,
                f"Rate limit exceeded: {len(minute_requests)} requests per minute",
                rule.block_duration,
            )
        ...
        if len(hour_requests) > max_per_hour:
            ...
            return (
                False,
                f"Rate limit exceeded: {len(hour_requests)} requests per hour",
                rule.block_duration * 2,
            )

        # Log threat escalation
        if client_metrics.threat_level != ThreatLevel.LOW:
            logger.warning(
                f"🚨 Elevated threat level {client_metrics.threat_level.value} for {client_ip}"
            )
    ...
    def _block_client(self, client_metrics: ClientMetrics, duration: int):
        ...
        logger.warning(
            f"🚫 Blocked {client_metrics.ip_address} for {duration} seconds due to rate limit violation"
        )
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "block_rate": (
                self.global_metrics["blocked_requests"]
...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, config: Optional[RateLimitingConfig] = None):
        ...
        logger.info(
            "🛡️ Rate limiting middleware initialized with adaptive DDoS protection"
        )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            # Check rate limits
            is_allowed, reason, retry_after = await self.rate_limiter.check_rate_limit(
                request
            )
            ...
            logger.debug(
                f"🛡️ Rate limit check passed for {client_ip} in {processing_time:.3f}s"
            )
