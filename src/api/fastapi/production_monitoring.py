    ...
    try:
        ...
        if health_result.get("status") == "healthy":
            ...
            if pool_utilization > 0.9:
                db_status = "degraded"
                issues.append(f"High connection pool utilization: {pool_utilization:.1%}")
...
async def comprehensive_health_check():
    ...
    try:
        ...
        if isinstance(system_health, Exception):
            system_health = {"status": "error", "error": str(system_health), "issues": ["System health check failed"]}
        if isinstance(db_health, Exception):
            db_health = {"status": "error", "error": str(db_health), "issues": ["Database health check failed"]}
        if isinstance(cache_health, Exception):
            cache_health = {"status": "error", "error": str(cache_health), "issues": ["Cache health check failed"]}
...
async def get_system_status():
    ...
    try:
        ...
        try:
            ...
            active_alerts = len([alert for alert in recent_alerts 
