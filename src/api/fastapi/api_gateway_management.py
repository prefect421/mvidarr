...
from src.services.api_gateway import get_api_gateway, ServiceInstance, RouteRule, RoutingStrategy
...
class RouteRuleRequest(BaseModel):
    ...
    routing_strategy: str = Field(default="round_robin", description="Load balancing strategy")
    ...
    rate_limit_requests: Optional[int] = Field(None, description="Rate limit per window")
    rate_limit_window_seconds: int = Field(default=60, description="Rate limit window")
    require_authentication: bool = Field(default=True, description="Authentication required")
    ...
    try:
        ...
        for service_name, instances in gateway.service_registry.services.items():
            ...
            for instance in instances:
                service_instances.append(ServiceResponse(
...
async def list_routes():
    ...
    try:
        ...
        for route in gateway.routing_rules:
            routes_data.append(RouteResponse(
...
async def delete_route_rule(rule_id: str):
    ...
    try:
        ...
        gateway.routing_rules = [
            rule for rule in gateway.routing_rules 
        ...
        if len(gateway.routing_rules) == original_count:
            raise HTTPException(
                status_code=404,
                detail=f"Route rule {rule_id} not found"
            )
...
async def get_migration_recommendations(version: str):
    ...
    try:
        ...
        recommendations = await versioning_service.get_migration_recommendations(version)
