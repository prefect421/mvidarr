...
class BatchDetectionRequest(BaseModel):
    ...
    include_non_music: bool = Field(False, description="Include non-music videos in results")

class OrganizationRequest(BaseModel):
    source_directory: str = Field(..., description="Source directory to organize")
    target_directory: str = Field(..., description="Target directory for organized files")
    ...
    preserve_quality_info: bool = Field(True, description="Preserve quality indicators in filenames")
    group_versions: bool = Field(True, description="Group different versions together")
    handle_duplicates: bool = Field(True, description="Handle duplicate detection during organization")
    create_artist_folders: bool = Field(True, description="Create artist-specific folders")
...
class DuplicateScanRequest(BaseModel):
    ...
    min_confidence: str = Field("medium", description="Minimum confidence level for duplicates")

class DuplicateRemovalRequest(BaseModel):
    group_ids: List[str] = Field(..., description="Duplicate group IDs to process")
    keep_strategy: str = Field("highest_quality", description="Strategy for which files to keep")
    dry_run: bool = Field(True, description="Preview changes without executing")
...
async def detect_batch_music_videos(request: BatchDetectionRequest, background_tasks: BackgroundTasks):
...
@collection_router.get("/detect/directory/{directory_path:path}", response_model=CollectionResponse)
    ...
    try:
        ...
        if not video_files:
            return CollectionResponse(
                ...
                data={'directory': directory_path, 'video_files_found': 0, 'music_videos': []},
...
async def create_organization_plan(request: OrganizationRequest):
    ...
    try:
        ...
        if not os.path.exists(request.source_directory):
            raise HTTPException(status_code=404, detail=f"Source directory not found: {request.source_directory}")
        ...
        rules.strategy = strategy_map.get(request.strategy, OrganizationStrategy.ARTIST_TITLE)
...
async def execute_organization(request: OrganizationRequest, background_tasks: BackgroundTasks):
...
async def scan_for_duplicates(request: DuplicateScanRequest):
    ...
    try:
        ...
        min_confidence = confidence_map.get(request.min_confidence, DuplicateConfidence.MEDIUM)
        
        # Perform duplicate scan
        scan_result = await duplicate_manager.scan_for_duplicates(
            request.directory,
            request.scan_subdirs,
            min_confidence
        )
...
async def remove_duplicates(request: DuplicateRemovalRequest, background_tasks: BackgroundTasks):
...
@collection_router.get("/stats/{directory_path:path}", response_model=CollectionResponse)
async def get_collection_statistics(directory_path: str):
    ...
    try:
        ...
        if stats['video_files'] > 0:
            ...
            folder_organization = min(stats['artist_folders'] / max(stats['video_files'] // 10, 1), 1.0)
            stats['organization_score'] = (organized_ratio * 0.7 + folder_organization * 0.3) * 100
...
async def get_supported_formats():
    ...
    return JSONResponse({
        ...
        'confidence_levels': ['very_low', 'low', 'medium', 'high', 'very_high'],
        'duplicate_strategies': ['highest_quality', 'largest_file', 'smallest_file', 'first_alphabetical']
    })
