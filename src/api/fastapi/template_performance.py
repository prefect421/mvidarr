...
class TemplateCache:
    ...
    async def set(self, template_name: str, context: Dict[str, Any], content: str, ttl: Optional[int] = None):
...
class PerformanceOptimizedRenderer:
    ...
    def __init__(self, template_system, cache_enabled: bool = True, compression_enabled: bool = True):
    ...
    async def render_template_optimized(self, template_name: str, request: Request, context: Dict[str, Any] = None) -> HTMLResponse:
        ...
        try:
            # Build full context
            full_context = await self.template_system.get_template_context(request, context)
                ...
                for hook in self.optimization_hooks:
                    try:
                        if asyncio.iscoroutinefunction(hook):
                            full_context = await hook(template_name, full_context) or full_context
                        else:
                            full_context = hook(template_name, full_context) or full_context
                    except Exception as e:
                        logger.warning(f"Optimization hook error: {e}")
                
                # Render template
                rendered_content = await self.template_system.render_template(template_name, request, full_context)
            ...
            # Compression
            if self.compression_enabled and TemplateCompressor.should_compress(cached_content):
                if "gzip" in request.headers.get("accept-encoding", ""):
                    compressed_content = TemplateCompressor.compress_html(cached_content)
                    response = Response(
            ...
            # Record metrics
            self._record_metrics(template_name, render_time, cache_hit, len(cached_content), full_context)
            ...
            return await self.template_system.render_response(template_name, request, context)
    
    def _record_metrics(self, template_name: str, render_time: float, cache_hit: bool, content_length: int, context: Dict[str, Any]):
    ...
    async def preload_templates(self, template_names: List[str], sample_context: Dict[str, Any] = None):
        ...
        for template_name in template_names:
            try:
                ...
                # Render and cache
                await self.render_template_optimized(template_name, request, sample_context)
...
# Utility functions
async def create_optimized_renderer(template_system, cache_enabled: bool = True, compression_enabled: bool = True) -> PerformanceOptimizedRenderer:
    """Create and initialize optimized renderer"""
    renderer = PerformanceOptimizedRenderer(template_system, cache_enabled, compression_enabled)
...
def add_static_content_hook(renderer: PerformanceOptimizedRenderer):
    ...
    async def optimize_static_content(template_name: str, context: Dict[str, Any]) -> Dict[str, Any]:
        # Add versioned static file URLs for better caching
        context['static_version'] = int(time.time())  # Simple versioning
        
        # Preload critical resources
        context['preload_resources'] = [
            '/static/css/main.css',
            '/static/js/core.js'
        ]
