...
class RequestResponseLogger:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.log_file_path = self.config.get('log_file', '/var/log/mvidarr/api_requests.log')
        ...
        self.sampling_rate = self.config.get('sampling_rate', 1.0)  # Log 100% by default
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info(f"API request logging initialized - Level: {self.log_level.value}")
    ...
    async def create_log_entry(
        ...
        log_entry = APILogEntry(
            ...
            request_body=request_data.get('body') if self.log_level == LogLevel.FULL else None,
            
            status_code=response_data['status_code'],
            response_size=int(response_data.get('content_length', 0)),
            response_body=response_data.get('body') if self.log_level == LogLevel.FULL else None,
            response_headers=response_data['headers'] if self.log_level != LogLevel.BASIC else {},
            
            processing_time_ms=processing_time,
            database_time_ms=additional_metrics.get('database_time_ms') if additional_metrics else None,
            cache_time_ms=additional_metrics.get('cache_time_ms') if additional_metrics else None,
            
            endpoint_name=additional_metrics.get('endpoint_name') if additional_metrics else None,
            errors=additional_metrics.get('errors', []) if additional_metrics else [],
            warnings=additional_metrics.get('warnings', []) if additional_metrics else [],
    ...
    async def _store_in_redis(self, log_entry: APILogEntry):
        ...
        try:
            ...
            await self.redis_client.setex(log_key, self.redis_retention_hours * 3600, log_data)
            
            # Add to time-ordered index
            timestamp = time.time()
            await self.redis_client.zadd("api_logs_timeline", {log_entry.request_id: timestamp})
            
            # Add to endpoint-specific index
            endpoint_key = f"api_logs_endpoint:{log_entry.path}"
            await self.redis_client.zadd(endpoint_key, {log_entry.request_id: timestamp})
    ...
    async def _update_performance_metrics(self, log_entry: APILogEntry):
        ...
        try:
            ...
            pipe.ltrim(f"{endpoint_key}:response_times", 0, 999)  # Keep last 1000 requests
    ...
    async def get_performance_summary(
        ...
        try:
            ...
            if endpoint:
                ...
                endpoints = [key.split(':', 2)[2] for key in keys if key.endswith(':count')]
            ...
            for ep in endpoints:
                ...
                response_times = await self.redis_client.lrange(f"{ep_key}:response_times", 0, -1)
