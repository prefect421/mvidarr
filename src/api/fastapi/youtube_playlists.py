...
async def get_monitored_playlists(
    ...
    try:
        ...
        logger.info(f"Getting monitored playlists for user {current_user.get('username')}")
...
async def create_playlist_monitor(
    ...
    try:
        ...
        logger.info(f"Creating playlist monitor for URL '{monitor_request.playlist_url}' by user {current_user.get('username')}")
        ...
        if not result.get("success"):
            raise HTTPException(status_code=400, detail=result.get("error", "Failed to create playlist monitor"))
...
async def get_playlist_monitor(
    ...
    try:
        ...
        logger.info(f"Getting playlist monitor {monitor_id} for user {current_user.get('username')}")
...
async def update_playlist_monitor(
    ...
    try:
        ...
        logger.info(f"Updating playlist monitor {monitor_id} for user {current_user.get('username')}")
...
async def delete_playlist_monitor(
    ...
    try:
        ...
        logger.info(f"Deleting playlist monitor {monitor_id} (delete_videos={delete_videos}) for user {current_user.get('username')}")
...
async def sync_playlist(
    ...
    try:
        ...
        logger.info(f"Syncing playlist '{playlist['name']}' (ID: {monitor_id}) for user {current_user.get('username')}")
...
async def sync_playlists(
    ...
    try:
        ...
        logger.info(f"Syncing playlists from settings page for user {current_user.get('username')}")
...
async def extract_playlist_id(
    ...
    try:
        ...
        playlist_id = youtube_playlist_service.extract_playlist_id(url_request.playlist_url)
...
async def get_playlist_info(
    ...
    try:
        ...
        logger.info(f"Getting playlist info for ID '{playlist_id}' for user {current_user.get('username')}")
...
async def preview_playlist_videos(
    ...
    try:
        ...
        playlist_id = youtube_playlist_service.extract_playlist_id(preview_request.playlist_url)
        
        if not playlist_id:
            raise HTTPException(status_code=400, detail="Invalid YouTube playlist URL")
        
        logger.info(f"Previewing playlist '{playlist_id}' for user {current_user.get('username')}")
        ...
        # Get videos
        videos = youtube_playlist_service.get_playlist_videos(playlist_id, preview_request.max_results)
...
async def test_youtube_integration(
    ...
    try:
        ...
        logger.info(f"Testing YouTube API connection for user {current_user.get('username')}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"YouTube API connection failed: {str(e)}"
        )
    except Exception as e:
        logger.error(f"YouTube integration test failed: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"YouTube integration test failed: {str(e)}"
        )