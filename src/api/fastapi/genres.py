...
def parse_genre_string(genre_string):
    ...
    try:
        ...
        logger.error(
            f"CRITICAL: parse_genre_string failed completely for {type(genre_string)}: {repr(genre_string)}, error: {e}"
        )
        return []
...
async def get_all_genres(
    ...
    try:
        ...
        # Get video genres
        videos = (
            session.query(Video.genres)
            .filter(Video.genres.isnot(None), Video.genres != "")
            .all()
        )
                ...
                for genre in genre_list:
                    if genre and genre.strip():
                        ...
                        if genre_clean not in genre_stats:
                            genre_stats[genre_clean] = {
                                "video_count": 0,
                                "artist_count": 0,
                                "original_case": genre.strip(),
                            }
                        genre_stats[genre_clean]["video_count"] += 1
            except Exception as e:
                logger.warning(
                    f"Failed to parse video genres: {genre_string}, error: {e}"
                )

        # Get artist genres
        artists = (
            session.query(Artist.genres)
            .filter(Artist.genres.isnot(None), Artist.genres != "")
            .all()
        )
        ...
        for (genre_string,) in artists:
            try:
                ...
                logger.warning(
                    f"Failed to parse artist genres: {genre_string}, error: {e}"
                )
...
async def get_popular_genres(
    ...
    try:
        ...
        # Sort and limit
        popular_genres = sorted(genre_stats.items(), key=lambda x: x[1], reverse=True)[
            :limit
        ]
        ...
        return {"genres": [], "total_count": 0, "limit": limit, "error": str(e)}
