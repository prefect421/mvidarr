...
class DiscoveryRequest(BaseModel):
    ...
    limit: int = Field(default=10, ge=1, le=50, description="Maximum videos to discover")


class BulkDiscoveryRequest(BaseModel):
    """Bulk video discovery request"""
    limit_per_artist: int = Field(default=5, ge=1, le=20, description="Maximum videos per artist")
...
async def discover_for_artist(
    ...
    try:
        ...
        logger.info(f"Discovering videos for artist '{artist.name}' (ID: {artist_id}) with limit {discovery_request.limit} for user {current_user.get('username')}")
...
async def discover_for_all_artists(
    ...
    try:
        ...
        logger.info(f"Discovering videos for all artists with limit {bulk_request.limit_per_artist} per artist for user {current_user.get('username')}")
        ...
        if not result.get("success"):
            raise HTTPException(
                status_code=400,
                detail=result.get("error", "Bulk discovery failed")
            )
...
async def get_discovery_stats(
    ...
    try:
        ...
        logger.info(f"Getting discovery statistics for user {current_user.get('username')}")
...
async def test_discovery_for_artist(
    ...
    try:
        ...
        logger.info(f"Testing video discovery for artist '{artist.name}' (ID: {artist_id}) for user {current_user.get('username')}")
        ...
        if not result.get("success"):
            raise HTTPException(
                status_code=400,
                detail=result.get("error", "Discovery test failed")
            )
