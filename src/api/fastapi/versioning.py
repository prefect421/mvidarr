...
class APIVersionManager:
    ...
    def get_latest_version(self) -> str:
        ...
        stable_versions = [
            v for v in self.versions.values() if v.status == APIVersionStatus.STABLE
        ]
        if stable_versions:
    ...
    def get_deprecation_warnings(self, requested_version: str) -> List[str]:
        ...
        if version_info:
            ...
            if version_info.status == APIVersionStatus.SUNSET:
                warnings.append(
                    f"API version {requested_version} is sunset and will be removed"
                )
...
def extract_version_from_request(request: Request) -> str:
    ...
    version_match = re.search(
        r"application/vnd\.mvidarr\.v(\d+(?:\.\d+)*)", accept_header
    )
    if version_match:
...
def add_version_headers(response, api_version: str):
    ...
    if warnings:
        ...
        response.headers["Sunset"] = (
            version_manager.get_version_info(api_version).sunset_date.isoformat()
            if version_manager.get_version_info(api_version).sunset_date
            else ""
        )
...
def create_version_info_router() -> APIRouter:
    ...
    async def get_version_info(api_version: str = Depends(get_api_version)):
        ...
        return {
            ...
            "deprecation_date": (
                version_info.deprecation_date.isoformat()
