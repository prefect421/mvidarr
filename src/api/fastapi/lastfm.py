...
class ImportArtistsRequest(BaseModel):
    ...
    limit: int = Field(default=50, ge=1, le=200, description="Number of artists to import")
    min_playcount: int = Field(default=1, ge=1, description="Minimum play count for import")


class ImportTracksRequest(BaseModel):
    username: Optional[str] = None
    limit: int = Field(default=200, ge=1, le=1000, description="Number of tracks to import")


class SyncHistoryRequest(BaseModel):
    username: Optional[str] = None
    limit: int = Field(default=200, ge=1, le=1000, description="Number of tracks to sync")
...
async def get_lastfm_status():
    ...
    try:
        ...
        try:
            ...
            configured = (lastfm_service.api_key is not None and 
                         lastfm_service.api_secret is not None)
        except Exception as e:
        ...
        return {
            ...
            "api_key": (lastfm_service.api_key[:8] + "..." 
                       if lastfm_service.api_key else None)
        }
...
async def test_lastfm_connection():
    ...
    try:
        ...
        # Test API connection
        try:
            result = lastfm_service.call_api("artist.search", {"artist": "test", "limit": 1})
            
            if result and "results" in result:
                return {
                    "status": "success",
                    "message": "Last.fm API connection successful",
                    "test_results": len(result["results"].get("artistmatches", {}).get("artist", []))
                }
            else:
                raise HTTPException(
                    status_code=500,
                    detail="Last.fm API returned unexpected response"
                )
        except Exception as e:
            raise HTTPException(
                status_code=500,
                detail=f"Last.fm API connection failed: {str(e)}"
            )
...
async def handle_callback(token: Optional[str] = Query(None)):
    ...
    try:
        ...
        if not token:
            raise HTTPException(status_code=400, detail="No authentication token received")
        ...
        logger.info(f"Last.fm authentication successful for user: {session_data.get('username')}")
...
async def get_profile(username: Optional[str] = Query(None)):
    ...
    try:
        ...
        if not username:
            raise HTTPException(status_code=400, detail="No username provided or not authenticated")
...
async def import_top_artists(request: ImportArtistsRequest):
    ...
    try:
        ...
        if not username:
            raise HTTPException(
                status_code=400,
                detail="No username provided or not authenticated"
            )
        ...
        logger.info(f"Starting import of top artists for user: {username}, period: {period}")
