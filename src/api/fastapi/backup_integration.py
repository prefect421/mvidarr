...
from src.services.personal_backup import get_personal_backup_service, CloudProvider, BackupType, VideoQuality
from src.integrations.youtube_importer import get_youtube_importer, ImportType
...
class CloudProviderConfigRequest(BaseModel):
    provider: str = Field(..., description="Cloud provider (google_drive, dropbox, onedrive)")
...
async def configure_cloud_provider(request: CloudProviderConfigRequest):
    ...
    try:
        ...
        result = await backup_service.configure_cloud_provider(provider, request.credentials)
...
async def create_backup_job(request: BackupJobRequest, background_tasks: BackgroundTasks):
...
async def get_backup_job_status(job_id: str):
    ...
    try:
        ...
        return IntegrationResponse(
            success=True,
            message="Backup job status retrieved",
            data=status
        )
...
async def create_youtube_import_job(request: YouTubeImportRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid quality: {request.quality}")
...
async def get_youtube_import_status(job_id: str):
    ...
    try:
        ...
        if not status:
            raise HTTPException(status_code=404, detail=f"YouTube import job {job_id} not found")
        
        return IntegrationResponse(
            success=True,
            message="YouTube import job status retrieved",
            data=status
        )
...
async def create_network_share(request: NetworkShareRequest):
    ...
    try:
        ...
        # Validate enums
        from src.services.local_network_share import ShareType, AccessLevel
...
async def get_network_sharing_status():
    ...
    try:
        ...
        return IntegrationResponse(
            success=True,
            message="Network sharing status retrieved",
            data=status
        )
...
async def get_integration_capabilities():
    ...
    return JSONResponse({
        ...
        'network_sharing': {
            ...
            'share_types': ['music_videos', 'collections', 'playlists', 'recent_imports', 'custom_folder']
        }
    })
