...
async def get_artist(
    ...
    try:
        ...
        if not result:
            raise HTTPException(status_code=404, detail="Artist not found in MusicBrainz")
...
async def get_artist_releases(
    ...
    try:
        ...
        result = await asyncio.to_thread(musicbrainz_service.get_artist_releases, mbid, limit=limit)
...
async def get_musicbrainz_status(
    ...
    try:
        if not musicbrainz_service:
            return {
                "available": False,
                "error": "MusicBrainz service not configured"
            }
