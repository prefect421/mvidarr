...
recommendations_router = APIRouter(
    prefix="/api/recommendations", tags=["Music Recommendations"]
)
...
class SpotifyRecommendationRequest(BaseModel):
    ...
    seed_artists: Optional[List[str]] = Field(
        default=None, description="Seed artist names or IDs"
    )
    seed_tracks: Optional[List[str]] = Field(
        default=None, description="Seed track names or IDs"
    )
...
class ArtistRecommendationRequest(BaseModel):
    ...
    max_recommendations: int = Field(
        default=10, description="Maximum recommendations to return"
    )
    ...
    try:
        ...
        recommendations_result = await get_music_recommendations(
            ...
            recommendation_types=[
                ...
                RecommendationType.DISCOVER_WEEKLY,
            ],
        ...
        await track_media_processing_time(
            "spotify_recommendations_api", recommendations_result.processing_time
        )

        return RecommendationResponse(
            recommendations=[
                rec.to_dict() for rec in recommendations_result.recommendations
            ],
...
async def generate_custom_spotify_recommendations(
    ...
    try:
        logger.info(
            f"🎵 Custom Spotify recommendations with seeds: {request.seed_artists}"
        )
        ...
        spotify_recs = await asyncio.to_thread(
            spotify_service.get_recommendations, **recommendation_params
        )
        ...
        for track in spotify_recs.get("tracks", []):
            ...
            try:
                ...
                recommendations.append(
                    {
                        ...
                        "source": "spotify",
                        "reasons": [
                            "Custom Spotify recommendation with audio features"
                        ],
                        "metadata": {
                            "spotify_track_id": track["id"],
                            "popularity": track.get("popularity", 0),
                            "audio_features": {
                                "danceability": track.get("audio_features", {}).get(
                                    "danceability"
                                ),
                                "energy": track.get("audio_features", {}).get("energy"),
                                "valence": track.get("audio_features", {}).get(
                                    "valence"
                                ),
                            },
                        },
                ...
                logger.warning(
                    f"Failed to find video for {artist_name} - {track_name}: {video_error}"
                )
        ...
        # Track performance
        await track_media_processing_time(
            "spotify_custom_recommendations_api", processing_time
        )
...
@recommendations_router.get(
    "/spotify/artist/{artist_id}", response_model=RecommendationResponse
)
async def get_spotify_artist_recommendations(
    artist_id: str, limit: int = Query(10, description="Maximum recommendations")
):
    """
    ...
    try:
        ...
        # Track performance
        await track_media_processing_time(
            "spotify_artist_recommendations_api", recommendations_result.processing_time
        )
...
async def get_artist_recommendations(request: ArtistRecommendationRequest):
    ...
    try:
        ...
        recommendations_result = await get_similar_artist_videos(
            request.artist_name, request.max_recommendations
        )

        # Track performance
        await track_media_processing_time(
            "artist_recommendations_api", recommendations_result.processing_time
        )
...
async def get_genre_recommendations(request: GenreRecommendationRequest):
    ...
    try:
        ...
        # Track performance
        await track_media_processing_time(
            "genre_recommendations_api", recommendations_result.processing_time
        )
...
async def get_trending_recommendations(
    ...
    try:
        ...
        # Track performance
        await track_media_processing_time(
            "trending_recommendations_api", recommendations_result.processing_time
        )
...
async def get_new_release_recommendations(
    ...
    try:
        ...
        # Track performance
        await track_media_processing_time(
            "new_release_recommendations_api", recommendations_result.processing_time
        )
...
async def get_recommendation_statistics():
    ...
    try:
        ...
        return RecommendationStatsResponse(
            ...
            successful_recommendations=stats["recommendation_stats"][
                "successful_recommendations"
            ],
            cache_hits=stats["recommendation_stats"]["cache_hits"],
            average_processing_time=stats["recommendation_stats"][
