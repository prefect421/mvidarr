...
frontend_router = APIRouter(
    tags=["frontend"], responses={404: {"description": "Page not found"}}
)
...
async def video_detail(request: Request, video_id: int):
    ...
    try:
        ...
        return await template_system.render_response(
            "video_detail.html", request, context
        )
    except Exception as e:
...
@frontend_router.get("/wizard", response_class=HTMLResponse, name="wizard_page")
async def wizard_page(request: Request):
    """Installation Wizard - First-run setup (no authentication required)"""
    try:
...
@frontend_router.get(
    "/auth/simple-login", response_class=HTMLResponse, name="auth_simple_login"
)
...
@frontend_router.get(
    "/auth/2fa/setup", response_class=HTMLResponse, name="auth_2fa_setup"
)
...
@frontend_router.get(
    "/admin/users/create", response_class=HTMLResponse, name="admin_create_user"
)
async def admin_create_user(request: Request, user=Depends(require_admin)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to load admin create user page"
        )


@frontend_router.get(
    "/admin/users/{user_id}", response_class=HTMLResponse, name="admin_user_details"
)
async def admin_user_details(
    request: Request, user_id: int = Path(...), admin_user=Depends(require_admin)
):
    ...
    try:
        ...
        return await template_system.render_response(
            "admin/user_details.html", request, context
        )
    except Exception as e:
...
async def add_video_modal_component(request: Request):
    ...
    try:
        ...
        return await template_system.render_response(
            "components/add_video_modal.html", request, context
        )
    except Exception as e:
...
async def job_dashboard_modal_component(request: Request):
    ...
    try:
        ...
        return await template_system.render_response(
            "components/job_dashboard_modal.html", request, context
        )
    except Exception as e:
        logger.error(f"Error rendering job dashboard modal: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to load job dashboard modal"
        )
...
async def frontend_health():
    ...
    try:
        ...
        return {
            "status": (
                "healthy"
                if all([template_status, static_status, templates_status])
                else "degraded"
            ),
        ...
        return {"status": "error", "error": str(e), "timestamp": "2025-01-08T00:00:00Z"}
...
async def not_found_handler(request: Request, exc):
    ...
    try:
        ...
        return await template_system.render_response(
            "errors/404.html", request, context
        )
    except Exception as e:
...
async def internal_server_error_handler(request: Request, exc):
    ...
    try:
        ...
        return await template_system.render_response(
            "errors/500.html", request, context
        )
    except Exception as e:
