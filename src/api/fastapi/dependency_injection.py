...
from typing import Dict, Any, Optional, Callable, Type, TypeVar, Generic, get_type_hints
...
class DependencyContainer:
    ...
    def _create_instance(self, key: str, interface: Type[T]) -> T:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Dependency injection failed: {e}")
                ...
                for validator in self.validation_rules[field]:
                    try:
                        if not validator(value):
                            field_errors.append(f"Validation failed for field '{field}'")
                    except Exception as e:
...
def register_default_services():
    ...
    validation_config = DependencyConfig(
        scope=DependencyScope.CACHED,
        cache_ttl=3600
    )
