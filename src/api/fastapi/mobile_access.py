...
async def mobile_search_videos(
    ...
    try:
        ...
        response = {
            ...
            'mobile_config': config,
            'suggestions': [f'{q} official', f'{q} live', f'{q} acoustic'] if len(q) > 2 else []
        }
...
async def download_video_mobile(
    ...
    try:
        ...
        if config['is_mobile'] and not config.get('enable_downloads', True):
            raise HTTPException(status_code=403, detail="Downloads not available on this device")
        ...
        return JSONResponse({
            ...
            'format': format,
            'mobile_optimized': True,
            'estimated_size_mb': 25 if quality == 'low' else 45 if quality == 'medium' else 85,
            'estimated_download_time_minutes': 2 if quality == 'low' else 4 if quality == 'medium' else 8,
            'expires_at': (datetime.now()).isoformat()  # Would be actual expiration
        })
