...
async def discover_artists_multi_source(
    ...
    try:
        ...
        logger.info(f"Enhanced discovery search for '{query}' by user {current_user.get('username')}")
...
async def enrich_artists_metadata(
    ...
    try:
        ...
        logger.info(f"Artist enrichment requested for {len(artist_ids)} artists by user {current_user.get('username')}")
        ...
        if missing_ids:
            raise HTTPException(
                status_code=400,
                detail=f"Artists not found: {list(missing_ids)}"
            )
        ...
        logger.info(f"Enrichment completed: {successful_count}/{len(results)} successful")
...
async def detect_duplicate_artists(
    ...
    try:
        ...
        logger.info(f"Duplicate detection requested with threshold {threshold} by user {current_user.get('username')}")
        ...
        logger.info(f"Duplicate detection found {len(results)} groups with {total_duplicates} potential duplicates")
...
async def get_artist_recommendations(
    ...
    try:
        ...
        logger.info(f"Recommendations requested for artist '{artist.name}' (ID: {artist_id}) by user {current_user.get('username')}")
        
        # Get recommendations using service
        recommended_artists = enhanced_artist_discovery_service.get_artist_recommendations(
            artist_id=artist_id,
            limit=limit,
            user_id=user_id
        )
        ...
        logger.info(f"Generated {len(results)} recommendations for artist '{artist.name}'")
...
async def get_discovery_statistics(
    ...
    try:
        ...
        logger.info(f"Discovery statistics requested for {days} days by user {current_user.get('username')}")
