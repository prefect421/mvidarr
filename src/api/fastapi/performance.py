...
class EndpointPerformance(BaseModel):
    ...
    error_rate: float = Field(description="Error rate percentage")
    cache_hit_rate: Optional[float] = Field(None, description="Cache hit rate for this endpoint")
...
async def get_performance_trends(
    ...
    try:
        ...
        return {
            ...
            "trends": trends,
            "summary": {
                "avg_response_time": sum(t.get('avg_response_time', 0) for t in trends) / max(len(trends), 1),
                "peak_requests_per_minute": max((t.get('requests_per_minute', 0) for t in trends), default=0),
                "avg_error_rate": sum(t.get('error_rate', 0) for t in trends) / max(len(trends), 1)
            }
        }
