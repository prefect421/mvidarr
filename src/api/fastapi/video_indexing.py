...
async def index_single_video(
    ...
    try:
        ...
        logger.info(f"Queueing single video indexing for '{index_request.file_path}' for user {current_user.get('username')}")
        ...
        logger.info(f"Enqueued single video indexing job {job_id} for {index_request.file_path}")
...
async def get_indexing_stats(
    ...
    try:
        ...
        logger.info(f"Getting indexing statistics for user {current_user.get('username')}")
...
async def scan_video_files(
    ...
    try:
        ...
        logger.info(f"Scanning video files (directory={directory}) for user {current_user.get('username')}")
...
async def preview_indexing(
    ...
    try:
        ...
        logger.info(f"Previewing indexing for '{preview_request.file_path}' for user {current_user.get('username')}")
        
        file_metadata = video_indexing_service.extract_file_metadata(Path(preview_request.file_path))
        ...
        if file_metadata.get("extracted_artist") and file_metadata.get("extracted_title"):
            try:
...
async def search_metadata(
    ...
    try:
        ...
        logger.info(f"Searching metadata for artist '{search_request.artist}' (title='{search_request.title}') for user {current_user.get('username')}")
        
        if search_request.title:
            # Search for specific video
            metadata = imvdb_service.find_best_video_match(search_request.artist, search_request.title)
            if metadata:
...
async def download_thumbnail(
    ...
    try:
        ...
        logger.info(f"Downloading thumbnail from '{download_request.url}' for user {current_user.get('username')}")
...
async def get_thumbnail_stats(
    ...
    try:
        ...
        logger.info(f"Getting thumbnail statistics for user {current_user.get('username')}")
