...
class JobRequest(BaseModel):
    ...
    retry_strategy: str = Field(default="exponential", description="exponential, linear, fixed")
    scheduled_for: Optional[datetime] = Field(default=None, description="When to execute the job")
    delay_seconds: Optional[int] = Field(default=None, description="Delay execution by seconds")
    depends_on: List[str] = Field(default_factory=list, description="Job IDs this job depends on")
    blocking: bool = Field(default=False, description="Whether this job blocks dependent jobs if it fails")
...
async def schedule_job(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid job type: {job_request.type}")
        ...
        if job_request.delay_seconds:
            job_id = await job_queue.schedule_job(job, delay_seconds=job_request.delay_seconds)
        else:
...
async def add_job_dependency(
    ...
    try:
        ...
        await job_queue.add_job_dependency(job_id, dependency.depends_on_job_id, dependency.blocking)
...
async def cancel_job(
    ...
    try:
        ...
        if job.status not in [JobStatus.QUEUED, JobStatus.SCHEDULED, JobStatus.WAITING_DEPENDENCIES]:
...
async def process_scheduled_jobs(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Scheduled jobs processing triggered"
        }
...
async def process_waiting_jobs(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Waiting jobs processing triggered"
        }
