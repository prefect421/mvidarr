...
async def get_certificate_status(
    ...
    try:
        ...
        logger.info(f"Getting certificate status for user {current_user.get('username')}")
...
async def upload_certificates(
    ...
    certificate_chain: Optional[UploadFile] = File(None, description="Certificate chain file (optional)"),
    current_user: dict = Depends(require_authentication_legacy),
    session: Session = Depends(get_db_session)
):
    ...
    try:
        ...
        if not is_valid:
            raise HTTPException(
                status_code=400,
                detail=f"Certificate validation failed: {error}"
            )
        ...
        if not is_valid:
            raise HTTPException(
                status_code=400,
                detail=f"Private key validation failed: {error}"
            )
...
async def remove_certificates(
    ...
    try:
        ...
        if not removed_files:
            raise HTTPException(
                status_code=404,
                detail="No certificate files found to remove"
            )
