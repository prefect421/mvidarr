...
class OptimizeRequest(BaseModel):
    ...
    dry_run: bool = Field(default=False, description="Run analysis without making changes")
...
async def get_search_performance(
    ...
    try:
        ...
        logger.info(f"Getting search performance stats for user {current_user.get('username')}")
...
async def analyze_thumbnail_storage(
    ...
    try:
        ...
        logger.info(f"Analyzing thumbnail storage for user {current_user.get('username')}")
...
async def get_optimization_recommendations(
    ...
    try:
        ...
        logger.info(f"Getting optimization recommendations for user {current_user.get('username')}")
        
        recommendations = thumbnail_optimization_service.get_optimization_recommendations()
...
async def optimize_thumbnails(
    ...
    try:
        ...
        logger.info(f"Optimizing thumbnails (dry_run={optimize_request.dry_run}) for user {current_user.get('username')}")
...
async def cleanup_duplicate_thumbnails(
    ...
    try:
        ...
        logger.info(f"Cleaning up duplicate thumbnails for user {current_user.get('username')}")
...
async def optimize_database_indexes(
    ...
    try:
        ...
        logger.info(f"Optimizing database indexes for user {current_user.get('username')}")
        ...
        if result:
            ...
            raise HTTPException(status_code=500, detail="Failed to optimize database indexes")
...
async def get_optimization_status(
    ...
    try:
        ...
        logger.info(f"Getting optimization status for user {current_user.get('username')}")
        ...
        status = OptimizationStatus(
            search_optimization={
                ...
                "cache_entries": search_stats.get("cache_stats", {}).get("active_entries", 0),
            },
            thumbnail_optimization={
                ...
                "optimization_potential": thumbnail_analysis.get("optimization_potential", 0),
            },
