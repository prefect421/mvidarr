...
class WebhookEndpointRequest(BaseModel):
    ...
    secret: Optional[str] = Field(None, max_length=256, description="Secret for webhook verification")
    events: List[str] = Field(default_factory=list, description="List of event types to subscribe to")
...
class WebhookTestRequest(BaseModel):
    ...
    secret: Optional[str] = Field(None, max_length=256, description="Secret for verification")
...
async def create_webhook(
    ...
    try:
        ...
        logger.info(f"Creating webhook endpoint for URL '{webhook_request.url}' by user {current_user.get('username')}")
...
async def update_webhook(
    ...
    try:
        ...
        logger.info(f"Updating webhook endpoint '{url}' for user {current_user.get('username')}")
...
async def delete_webhook(
    ...
    try:
        ...
        logger.info(f"Deleting webhook endpoint '{url}' for user {current_user.get('username')}")
...
async def test_webhook(
    ...
    try:
        ...
        logger.info(f"Testing webhook endpoint '{test_request.url}' for user {current_user.get('username')}")
        
        result = webhook_service.test_endpoint(str(test_request.url), test_request.secret)
...
async def validate_webhook(
    ...
    try:
        ...
        logger.info(f"Validating webhook configuration for user {current_user.get('username')}")
...
async def get_event_types(
    ...
    try:
        ...
        logger.info(f"Getting webhook event types for user {current_user.get('username')}")
...
async def trigger_webhook(
    ...
    try:
        ...
        logger.info(f"Triggering webhook event '{trigger_request.event_type}' for user {current_user.get('username')}")
...
async def get_webhook_stats(
    ...
    try:
        ...
        logger.info(f"Getting webhook statistics for user {current_user.get('username')}")
