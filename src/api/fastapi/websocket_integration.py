...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, client_id: str) -> WebSocketConnection:
        ...
        await self.send_to_client(client_id, WebSocketMessage(
            ...
            data={"client_id": client_id, "server_time": datetime.utcnow().isoformat()}
        ))
    ...
    async def disconnect(self, client_id: str):
        ...
        if client_id in self.connections:
            ...
            self.subscriptions = {
                topic: subscribers for topic, subscribers in self.subscriptions.items()
                if subscribers
            }
    ...
    async def send_to_client(self, client_id: str, message: WebSocketMessage):
        ...
        try:
            connection = self.connections[client_id]
            await connection.websocket.send_text(json.dumps(message.to_dict(), default=str))
            return True
    ...
    async def broadcast(self, message: WebSocketMessage, exclude_client: Optional[str] = None):
    ...
    async def handle_message(self, client_id: str, raw_message: str):
        ...
        try:
            ...
            if not message_type:
                await self.send_to_client(client_id, WebSocketMessage(
                    type="error",
                    data={"error": "Message type required"}
                ))
                return
        ...
        while self.connections:
            try:
                ...
                for client_id, connection in self.connections.items():
                    ...
                    inactive_duration = (current_time - connection.last_ping).total_seconds()
                    
                    if inactive_duration > timeout_threshold:
                        logger.warning(f"Client {client_id} timed out after {inactive_duration}s")
                        disconnected_clients.append(client_id)
...
class BackgroundJobMonitor:
    ...
    def __init__(self, websocket_manager: WebSocketManager):
        ...
        websocket_manager.register_message_handler('job_status_request', self.handle_job_status_request)
        websocket_manager.register_message_handler('job_cancel_request', self.handle_job_cancel_request)
...
class NotificationService:
    ...
    def __init__(self, websocket_manager: WebSocketManager):
        ...
        # Register message handlers
        websocket_manager.register_message_handler('notification_settings', self.handle_notification_settings)
    ...
    async def handle_notification_settings(self, client_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            for notification_type, enabled in settings.items():
                if enabled:
                    await self.websocket_manager.subscribe(client_id, f"notifications_{notification_type}")
                else:
                    await self.websocket_manager.unsubscribe(client_id, f"notifications_{notification_type}")
    ...
    async def send_notification(self, notification_type: str, title: str, message: str, data: Dict[str, Any] = None):
...
class SearchService:
    ...
    def __init__(self, websocket_manager: WebSocketManager):
        ...
        # Register message handlers
        websocket_manager.register_message_handler('search_request', self.handle_search_request)
...
async def send_system_notification(title: str, message: str, notification_type: str = "info"):
    """Send system-wide notification"""
    await notification_service.send_notification(notification_type, title, message)

async def broadcast_job_update(job_id: str, status: str, progress: int = None, message: str = None):
