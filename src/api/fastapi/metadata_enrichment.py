...
async def get_enrichment_candidates(
    ...
    try:
        ...
        for artist in artists:
            ...
            missing_ids = []
            if not artist.spotify_id or (isinstance(artist.spotify_id, str) and not artist.spotify_id.strip()):
                missing_ids.append("spotify_id")
            if not artist.lastfm_name or (isinstance(artist.lastfm_name, str) and not artist.lastfm_name.strip()):
                missing_ids.append("lastfm_name")
            if not artist.imvdb_id or (isinstance(artist.imvdb_id, str) and not artist.imvdb_id.strip()):
            ...
            if not musicbrainz_id or (isinstance(musicbrainz_id, str) and not musicbrainz_id.strip()):
...
async def enrich_single_artist(
    ...
    try:
        ...
        if not artist.spotify_id or (isinstance(artist.spotify_id, str) and not artist.spotify_id.strip()):
            try:
                if spotify_service:
                    spotify_result = await asyncio.to_thread(spotify_service.search_artist, artist.name)
                    if spotify_result and isinstance(spotify_result, dict):
                        ...
                        if spotify_id:
                            ...
                            matches_found["spotify"] = True
                            updated_fields.append("spotify_id")
                            logger.info(f"Found Spotify match for {artist.name}: {spotify_id}")
            except Exception as e:
        ...
        if not artist.lastfm_name or (isinstance(artist.lastfm_name, str) and not artist.lastfm_name.strip()):
            try:
                if lastfm_service:
                    lastfm_result = await asyncio.to_thread(lastfm_service.search_artist, artist.name)
                    if lastfm_result and isinstance(lastfm_result, dict):
                        ...
                        if lastfm_name:
                            ...
                            matches_found["lastfm"] = True
                            updated_fields.append("lastfm_name")
                            logger.info(f"Found Last.fm match for {artist.name}: {lastfm_name}")
            except Exception as e:
        ...
        if not existing_mbid:
            try:
                if musicbrainz_service:
                    mb_result = await asyncio.to_thread(musicbrainz_service.search_artist, artist.name)
                    if mb_result and isinstance(mb_result, dict):
                        mb_id = mb_result.get("mbid") or mb_result.get("id") or mb_result.get("musicbrainz_id")
                        if mb_id:
                            if not artist.imvdb_metadata:
                                artist.imvdb_metadata = {}
                            artist.imvdb_metadata["musicbrainz_id"] = mb_id
                            matches_found["musicbrainz"] = True
                            updated_fields.append("musicbrainz_id")
                            logger.info(f"Found MusicBrainz match for {artist.name}: {mb_id}")
            except Exception as e:
        ...
        if not artist.imvdb_id or (isinstance(artist.imvdb_id, str) and not artist.imvdb_id.strip()):
            try:
                if imvdb_service:
                    imvdb_result = await asyncio.to_thread(imvdb_service.search_artist, artist.name)
                    if imvdb_result and isinstance(imvdb_result, dict):
                        imvdb_id = imvdb_result.get("id") or imvdb_result.get("imvdb_id")
                        if imvdb_id:
                            ...
                            matches_found["imvdb"] = True
                            updated_fields.append("imvdb_id")
                            logger.info(f"Found IMVDb match for {artist.name}: {imvdb_id}")
            except Exception as e:
        ...
        if updated_fields:
            ...
            artist.updated_at = datetime.utcnow()
            session.commit()
            logger.info(f"Successfully enriched {artist.name} with fields: {', '.join(updated_fields)}")
        else:
