...
from src.services.collection_organizer import get_collection_organizer, OrganizationRule, OrganizationStrategy
from src.services.duplicate_manager import get_duplicate_manager, DuplicateConfidence
...
class BatchMusicVideoDetectionTask(CollectionOptimizationTask):
    ...
    async def execute(self, directory_path: str, options: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(0, total_files, batch_size):
                ...
                logger.info(f"Batch {batch_num}/{total_batches}: {batch_music_videos} music videos found")
            ...
            result = {
                ...
                'detection_summary': {
                    'very_high_confidence': sum(1 for r in detection_results if r.is_music_video and r.confidence.value == 'very_high'),
                    'high_confidence': sum(1 for r in detection_results if r.is_music_video and r.confidence.value == 'high'),
                    'medium_confidence': sum(1 for r in detection_results if r.is_music_video and r.confidence.value == 'medium'),
                    'low_confidence': sum(1 for r in detection_results if r.is_music_video and r.confidence.value == 'low')
                },
                'cache_key': cache_key
            }
            
            await self.update_progress(100, "Detection task completed")
            logger.info(f"Batch detection completed: {music_videos_detected} music videos detected from {total_files} files")
    ...
    async def _update_database_with_detections(self, detection_results):
        ...
        try:
            ...
            logger.info(f"Would update database with {len(music_videos)} music video detections")
...
class CollectionOrganizationTask(CollectionOptimizationTask):
    ...
    async def execute(self, source_directory: str, target_directory: str, options: Optional[Dict] = None) -> Dict[str, Any]:
        """Execute collection organization"""
        try:
            options = options or {}
            
            logger.info(f"Starting collection organization: {source_directory} -> {target_directory}")
            ...
            rules.strategy = strategy_map.get(options.get('strategy', 'artist_title'), OrganizationStrategy.ARTIST_TITLE)
            ...
            plan = await organizer.create_organization_plan(source_directory, target_directory, rules)
            ...
            await self.update_progress(20, f"Organizing {len(plan.organization_actions)} music videos")
            ...
            logger.info(f"Collection organization completed: {result.files_processed} files processed")
...
class DuplicateDetectionTask(CollectionOptimizationTask):
    ...
    async def execute(self, directory_path: str, options: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            confidence_level = confidence_map.get(min_confidence, DuplicateConfidence.MEDIUM)
            ...
            result = {
                ...
                'potential_space_savings_mb': scan_result.potential_space_savings / (1024 * 1024),
                'scan_time_seconds': scan_result.scan_time_seconds,
                'confidence_summary': {
                    'very_high': len(scan_result.get_groups_by_confidence(DuplicateConfidence.VERY_HIGH)),
                    'high': len(scan_result.get_groups_by_confidence(DuplicateConfidence.HIGH)),
                    'medium': len(scan_result.get_groups_by_confidence(DuplicateConfidence.MEDIUM)),
                    'low': len(scan_result.get_groups_by_confidence(DuplicateConfidence.LOW))
                },
                'cache_key': cache_key
            }
            
            await self.update_progress(100, "Duplicate detection completed")
            logger.info(f"Duplicate detection completed: {scan_result.duplicate_groups_found} groups, "
...
class CollectionHealthCheckTask(CollectionOptimizationTask):
    ...
    async def execute(self, directory_path: str, options: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            organization_issues = await self._check_organization_issues(directory_path, structure_analysis)
            ...
            health_report['overall_score'] = await self._calculate_health_score(health_report)
            ...
            logger.info(f"Collection health check completed: {health_report['overall_score']:.1f}/100 score")
            ...
            if structure['folder_count'] > 0:
                ...
                for item in directory_path.iterdir():
                    if item.is_dir() and not item.name.startswith('.'):
                        ...
                        if len(item.name) > 2 and not any(char.isdigit() for char in item.name[:4]):
                            artist_folders += 1
    ...
    async def _check_organization_issues(self, directory: str, structure: Dict) -> List[Dict]:
        """Check for organization issues"""
        issues = []
        
        # Check for poor organization
        if structure['organization_type'] == 'unknown' and structure['video_files'] > 20:
            issues.append({
    ...
    async def _generate_health_recommendations(self, health_report: Dict) -> List[Dict]:
        ...
        if 'poor_organization' in issues_by_type:
            recommendations.append({
                ...
                'benefits': ['Easier navigation', 'Better file management', 'Improved search']
            })
...
class CollectionOptimizationScheduler:
    ...
    async def schedule_periodic_optimization(self, directory: str, schedule_options: Dict):
            ...
            for key in keys:
                try:
                    ...
                    if await self._should_run_optimization(schedule_data):
                        ...
                        await self.redis_client.setex(key, 86400 * 7, json.dumps(schedule_data))
    ...
    async def _execute_scheduled_optimization(self, schedule_data: Dict):
        ...
        try:
            ...
            if options.get('duplicate_detection', False):
                ...
                await duplicate_task.execute(directory, {
