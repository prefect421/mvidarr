...
class StructuredLogger:
    ...
    def performance(self, operation: str, duration: float, **kwargs):
        ...
        self._log_with_context(logging.INFO, f"PERF: {operation} completed in {duration:.3f}s", **kwargs)
    
    def api_request(self, method: str, endpoint: str, status_code: int, duration: float, **kwargs):
        """API request logging"""
        kwargs.update({
        ...
        self._log_with_context(logging.INFO, f"API {method} {endpoint} -> {status_code} ({duration:.3f}s)", **kwargs)
