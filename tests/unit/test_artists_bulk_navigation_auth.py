"""Regression test for a residual auth gap in artists_bulk.py (#392 Phase 2
follow-up). Every other route in this file already requires
authentication; get_artist_navigation (GET /{artist_id}/navigation) was
the one route missed -- same tier as its siblings, no admin split needed.
"""
...
from src.api.fastapi.artists_bulk import router
from src.api.fastapi.auth_dependencies import require_authentication
from src.database.connection import get_db_session

SOURCE_PATH = (
    Path(__file__).parent.parent.parent / "src" / "api" / "fastapi" / "artists_bulk.py"
)
...
class TestArtistNavigationRequiresAuth:
    def test_route_requires_authentication(self):
        source = _function_source("get_artist_navigation")
...
class TestArtistNavigationBehavioralAuth:
    def _client(self):
        app = FastAPI()
        app.include_router(router)
        # get_artist_navigation declares session: Depends(get_db_session)
        ...
        # mask the 401 under test here. Matches real production behavior,
    ...
    def test_route_401s_without_session(self):
        client = self._client()
        response = client.get("/1/navigation")
